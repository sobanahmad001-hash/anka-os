-- P9-CHAT-3 conversational answers executable verifier. This script always rolls back.
begin;

create temporary table p9_chat3_answer_checks (
  check_name text primary key,
  passed boolean not null,
  detail text not null
) on commit drop;
grant select, insert on p9_chat3_answer_checks to service_role, authenticated;
...
declare
  ...
  v_constraint text;
begin
  insert into p9_chat3_answer_checks values
  ...
  ('answer_schema_acl',
    pg_get_constraintdef((select oid from pg_constraint
      where conrelid='public.ai_runs'::regclass and conname='ai_runs_capability_check')) like '%department_chat_answer%'
    and pg_get_constraintdef((select oid from pg_constraint
      where conrelid='public.department_chat_messages'::regclass
        and conname='department_chat_messages_terminal_check')) like '%role%assistant%'
    and to_regprocedure('public.complete_department_chat_answer(uuid,uuid,uuid,uuid,uuid,text,uuid,uuid,uuid,text,jsonb,text,integer,integer,integer,bigint)') is not null
    and not exists (
      select 1 from pg_proc procedure
      cross join lateral aclexplode(coalesce(procedure.proacl,acldefault('f',procedure.proowner))) privilege
      where procedure.oid=to_regprocedure('public.complete_department_chat_answer(uuid,uuid,uuid,uuid,uuid,text,uuid,uuid,uuid,text,jsonb,text,integer,integer,integer,bigint)')
        and privilege.grantee=0 and privilege.privilege_type='EXECUTE'
    )
    and not has_function_privilege('anon','public.complete_department_chat_answer(uuid,uuid,uuid,uuid,uuid,text,uuid,uuid,uuid,text,jsonb,text,integer,integer,integer,bigint)','EXECUTE')
    and not has_function_privilege('authenticated','public.complete_department_chat_answer(uuid,uuid,uuid,uuid,uuid,text,uuid,uuid,uuid,text,jsonb,text,integer,integer,integer,bigint)','EXECUTE')
    and has_function_privilege('service_role','public.complete_department_chat_answer(uuid,uuid,uuid,uuid,uuid,text,uuid,uuid,uuid,text,jsonb,text,integer,integer,integer,bigint)','EXECUTE'),
    'Answer capability, assistant completion shape, and service-role-only invoker RPC exist.');
end;
$catalog$;
...
insert into p9_chat3_answer_checks
...
begin
  ...
  from public.department_chat_model_configurations
  where organization_id=f.organization_id and connector_connection_id=f.connector_id
    and department_id='content' and model_id='gpt-other' and revoked_at is null;
  select to_jsonb(public.create_department_chat_conversation(
    f.organization_id,f.project_id,f.engagement_id,'content',f.contributor_id,'Answer verifier'))
  into conversation;
  turn := public.begin_department_chat_turn_with_attachments(
    (conversation->>'id')::uuid,f.organization_id,f.project_id,f.engagement_id,
    'content',f.contributor_id,gen_random_uuid(),'Explain only','{}'::uuid[]);
  perform public.assert_department_chat_model_dispatch(
    configuration_id,f.organization_id,f.engagement_id,'content',
    f.connector_id,'gpt-other',f.contributor_id);
  perform public.mark_department_chat_turn_dispatched(
    (turn->'message'->>'id')::uuid,(conversation->>'id')::uuid,
    f.organization_id,f.project_id,f.engagement_id,'content',f.contributor_id);
  perform public.configure_department_chat_model_allowlist(
    f.organization_id,f.connector_id,f.leader_id,'{"content":[]}'::jsonb);

  select count(*) into before_proposals from public.department_chat_proposals;
  select count(*) into before_work_items from public.work_items;
  select count(*) into before_artifacts from public.artifacts;
  select count(*) into before_runs from public.ai_runs;
  saved := public.complete_department_chat_answer(
    (conversation->>'id')::uuid,(turn->'message'->>'id')::uuid,
    f.organization_id,f.project_id,f.engagement_id,'content',f.contributor_id,
    configuration_id,f.connector_id,'gpt-other',
    jsonb_build_object(
      'department_id','content','connector_connection_id',f.connector_id,
      'model_configuration_id',configuration_id,'model_id','gpt-other',
      'context_checksum',repeat('a',64)
    ),'Verifier ordinary answer',12,3,4,0);
  insert into p9_chat3_answer_checks values
  ('answer_runtime_no_official_side_effects',
    (saved->>'replayed')::boolean=false
    and (select count(*) from public.department_chat_proposals)=before_proposals
    and (select count(*) from public.work_items)=before_work_items
    and (select count(*) from public.artifacts)=before_artifacts
    and (select count(*) from public.ai_runs)=before_runs+1
    and exists(select 1 from public.ai_runs where id=(saved->>'ai_run_id')::uuid
      and capability='department_chat_answer' and status='completed'
      and human_decision='not_applicable' and proposed_action is null
      and input_text='' and output_text=''
      and department_chat_model_configuration_id=configuration_id)
    and exists(select 1 from public.department_chat_messages
      where id=(saved->>'assistant_message_id')::uuid and role='assistant'
        and body='Verifier ordinary answer' and proposal_id is null)
    and exists(select 1 from public.department_chat_messages
      where id=(saved->>'user_message_id')::uuid and role='user'
        and status='completed' and proposal_id is null),
    'A dispatched answer survives later model revocation and writes only canonical messages plus one immutable AI-run ledger row.');

  replayed := public.complete_department_chat_answer(
    (conversation->>'id')::uuid,(turn->'message'->>'id')::uuid,
    f.organization_id,f.project_id,f.engagement_id,'content',f.contributor_id,
    configuration_id,f.connector_id,'gpt-other',
    jsonb_build_object(
  ...
  insert into p9_chat3_answer_checks values
  ('answer_exact_completion_replay',
    (replayed->>'replayed')::boolean
      and replayed->>'ai_run_id'=saved->>'ai_run_id'
      and (select count(*) from public.ai_runs)=before_runs+1
      and (select count(*) from public.department_chat_messages
        where conversation_id=(conversation->>'id')::uuid)=2,
    'Worker completion replay returns the same durable result without duplicate run or messages.');

  begin
    perform public.complete_department_chat_answer(
      (conversation->>'id')::uuid,(turn->'message'->>'id')::uuid,
      f.organization_id,f.project_id,f.engagement_id,'content',f.contributor_id,
      configuration_id,f.connector_id,'gpt-other',
      jsonb_build_object(
        ...
        'model_configuration_id',configuration_id,'model_id','gpt-other',
        'context_checksum',repeat('a',64)
      ),'Different replay answer',12,3,4,0);
  exception when check_violation then denied := true;
  end;
  insert into p9_chat3_answer_checks values
  ('answer_conflicting_completion_denied',denied
    and (select count(*) from public.ai_runs)=before_runs+1,
    'A conflicting completion replay is rejected without duplicate ledger or history writes.');
end;
$answer_runtime$;
reset role;
...
begin
  ...
  into failures from p9_chat3_answer_checks where not passed;
  if failures is not null then
    raise exception 'P9-CHAT-3 verification failed:%',E'\n' || failures;
  end if;
end;
$final$;

select check_name,passed,detail
from p9_chat3_answer_checks
order by check_name;
