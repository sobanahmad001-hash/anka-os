...
$cluster = Join-Path $parent.FullName ('anka-n6-ai-output-review-' + [guid]::NewGuid().ToString('N'))
...
try {
  ...
  $started = $true
  Invoke-LocalSql (Join-Path $PSScriptRoot 'n6-ai-output-review-fixture.sql')
  ...
  Invoke-LocalSql (Join-Path $PSScriptRoot '..\migrations\20260922190000_n6_ai_output_review.sql')
  Invoke-LocalSql (Join-Path $PSScriptRoot 'n6-ai-outcome-behavior.sql')
  Invoke-LocalSql (Join-Path $PSScriptRoot 'n6-ai-output-review-behavior.sql')
  Write-Output "N6 isolated AI-output-review checks passed. Cluster: $cluster"
}
finally {
  if ($started) {
