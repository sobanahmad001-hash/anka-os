...
try {
  ...
  foreach ($file in @(
    ...
    (Join-Path $PSScriptRoot 'n3_handoff.fixture.sql'),
    (Join-Path $PSScriptRoot '../migrations/20260920022000_n3_project_handoff_sources.sql'),
    (Join-Path $PSScriptRoot 'n3_handoff.behavior.sql'),
    (Join-Path $PSScriptRoot 'n3_discussion_links.fixture.sql'),
    (Join-Path $PSScriptRoot '../migrations/20260920023000_n3_project_discussion_links.sql'),
    (Join-Path $PSScriptRoot 'n3_discussion_links.behavior.sql')
  )) {
  ...
  Write-Output 'N3 local discussion, task proposal, handoff, and scoped reference checks passed.'
} finally {
  if ($started) {
