-- M05: atomic Marketing report save and immutable operation receipt.
begin;
set local lock_timeout='5s';
set local statement_timeout='120s';

create table private.m05_report_save_requests (
...
create index m05_report_save_actor_idx on private.m05_report_save_requests(organization_id,actor_id,created_at desc);
alter table private.m05_report_save_requests enable row level security;
revoke all on private.m05_report_save_requests from public,anon,authenticated,service_role;
grant select,insert on private.m05_report_save_requests to service_role;
create trigger m05_report_save_requests_immutable before update or delete on private.m05_report_save_requests
for each row execute function private.p7_reject_history_mutation();

create function public.save_marketing_report_version(
...
begin
  ...
  insert into private.m05_report_save_requests(request_id,organization_id,engagement_id,actor_id,payload_sha256,
    ...
    values(p_request_id,p_organization_id,p_engagement_id,p_actor_id,payload_sha,artifact.id,version.id);
  return to_jsonb(version)||jsonb_build_object('artifact_id',artifact.id,'request_id',p_request_id,'replayed',false);
end;
$$;
revoke all on function public.save_marketing_report_version(uuid,uuid,uuid,uuid,text,jsonb,text,text,boolean,uuid,uuid)
  from public,anon,authenticated,service_role;
grant execute on function public.save_marketing_report_version(uuid,uuid,uuid,uuid,text,jsonb,text,text,boolean,uuid,uuid)
  to service_role;
commit;
