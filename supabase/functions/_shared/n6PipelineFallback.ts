...
type ProviderResult = { ok: boolean; status: number; body: Record<string, any>; requestId: string }
type FallbackClaim = {
  status: string
...
> {
  ...
  for (let index = 0; index < contexts.length; index += 1) {
    ...
    const code = confirmedRetryableRejection(response.status, response.body)
    if (!code) {
