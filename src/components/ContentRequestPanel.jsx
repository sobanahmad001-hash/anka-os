...
export default function ContentRequestPanel({ engagement, repository }) {
  ...
  async function submit(event) {
    ...
    try {
      if (form.format !== 'article' && form.output_path === 'internal_engine' && form.media_type === 'image' && !form.model_registry_id) {
      ...
      if (form.format !== 'article' && form.output_path === 'internal_engine') {
      ...
      } else if (form.format !== 'article') await repository.ensureFigmaHandoff(createdRequest.id)
      setMessage(form.format === 'article' ? 'Article request created as a writing brief and is available for keyword targeting.' : form.output_path === 'internal_engine'
  ...
  return <div className="grid gap-6 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
    <form onSubmit={submit} className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6">
      <div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-amber-400">Project-mode request</p><h2 className="mt-1 text-2xl font-semibold">Create production content</h2><p className="mt-2 text-sm leading-6 text-slate-400">Create one article, reel, post, story set, or design element. Event context is always optional; routine content follows the same path.</p></div>
...
function RequestCard({ request, assets, handoff, event, onRepair }) {
  return <article className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
    ...
    <div className="mt-4 flex flex-wrap gap-2 text-[11px] text-slate-500"><span>{request.format === 'article' ? 'Writing brief' : request.output_path === 'internal_engine' ? 'Anka OS media' : 'Figma handoff'}</span><span>·</span><span>{event ? `Event: ${event.event_name}` : 'No event linked'}</span><span>·</span><span>{new Date(request.created_at).toLocaleString()}</span></div>
