...
function RequestCard({ request, assets, handoff, event, onRepair }) {
  return <article className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-amber-400">{request.format.replaceAll('_', ' ')}</p><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-300">{request.brief}</p></div><span className="rounded-full bg-slate-950 px-2.5 py-1 text-[10px] font-semibold uppercase text-slate-300">{request.status.replaceAll('_', ' ')}</span></div><div className="mt-4 flex flex-wrap gap-2 text-[11px] text-slate-500"><span>{request.output_path === 'internal_engine' ? 'Anka OS media' : 'Figma handoff'}</span><span>·</span><span>{event ? `Event: ${event.event_name}` : 'No event linked'}</span><span>·</span><span>{new Date(request.created_at).toLocaleString()}</span></div>{request.output_path === 'figma_handoff' && (handoff ? <div className="mt-4 flex flex-wrap items-center gap-3 rounded-xl border border-amber-900/50 bg-amber-950/20 p-3"><a href={handoff.figma_handoff_url} className="text-sm font-semibold text-amber-300 hover:text-amber-200">Open authenticated Figma reference →</a><span className="text-xs text-slate-500">No Figma API or file creation</span></div> : <button type="button" onClick={onRepair} className={`${BUTTON} mt-4`}>Create missing handoff page</button>)}{assets.length > 0 && <div className="mt-4 grid gap-3 sm:grid-cols-2">{assets.map(asset => <div key={asset.id} className="overflow-hidden rounded-xl border border-slate-800 bg-slate-950/60">{asset.signed_url && <img src={asset.signed_url} alt="Generated content request output" className="aspect-video w-full object-cover" />}<div className="p-3"><div className="flex items-center justify-between gap-2"><p className="text-xs font-semibold capitalize text-white">{asset.media_type}</p><span className={`rounded-full px-2 py-1 text-[10px] font-semibold uppercase ${TONES[mediaStatusTone(asset.status)]}`}>{asset.status}</span></div>{asset.failure_reason && <p className="mt-2 text-xs leading-5 text-red-300">{asset.failure_reason}</p>}</div></div>)}</div>}</article>
}
