...
export default function ArtifactRelationsPanel({ artifact }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [relationRows, candidateRows] = await Promise.all([
  ...
  const filteredCandidates = useMemo(() => {
    ...
    return candidates.filter(candidate => `${candidate.title} ${candidate.artifact_type}`.toLowerCase().includes(query))
  }, [candidates, search])
  ...
  return <section className="mt-6 rounded-2xl border border-cyan-500/20 bg-slate-950/55 p-5">
    ...
    <div className="mt-5 grid gap-5 lg:grid-cols-2">
      <RelationList title="This artifact relates to" direction="Outgoing" items={grouped.outgoing} loading={loading} onRemove={remove} />
      <RelationList title="Artifacts relating here" direction="Incoming" items={grouped.incoming} loading={loading} onRemove={remove} />
    </div>
      ...
      <div className="mt-3 grid gap-3 lg:grid-cols-[1fr_180px_auto]">
        <div className="space-y-2">
          ...
          <select required className={INPUT} value={targetId} onChange={event => setTargetId(event.target.value)}>
            ...
            {filteredCandidates.map(candidate => <option key={candidate.id} value={candidate.id}>{candidate.title} · {artifactTypeLabel(candidate.artifact_type)}</option>)}
          </select>
        </div>
