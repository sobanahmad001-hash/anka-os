...
export default function DesignAssetLibrary({ workspace, contextKey, onClose, onFocusSource }) {
  ...
  return <section aria-labelledby="design-asset-library-title" className="rounded-2xl border border-violet-400/20 bg-slate-900/80 p-4 shadow-xl shadow-black/10 sm:p-5">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div><p className="text-xs font-semibold uppercase tracking-wider text-violet-300">Design S05 · Read-only</p><h2 id="design-asset-library-title" className="mt-2 text-2xl font-semibold">Browse assets</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">These are already-loaded outputs from the current authorized Design context. Browsing never creates, regenerates, edits, approves, or archives an asset.</p></div>
    ...
    {selected && <aside aria-labelledby="design-asset-detail-title" className="mt-6 rounded-2xl border border-white/10 bg-slate-950/70 p-4 sm:p-5">
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,1fr)]">
        ...
        <dl className="grid content-start gap-3 text-sm sm:grid-cols-2">
          ...
          <Metadata term="Source">{selected.sourceType === 'variant' ? 'Recorded variant · ' + label(selected.recordedVariantFormat) : selected.sourceType === 'generated' ? 'Durable generated output' : 'Other recorded output'}</Metadata><Metadata term="Created">{selected.createdAt ? new Date(selected.createdAt).toLocaleString() : 'Unavailable / not recorded'}</Metadata>
          ...
          <Metadata term="Dimensions">Unavailable / not recorded</Metadata><Metadata term="MIME / file format">Unavailable / not recorded</Metadata>
          <Metadata term="Asset name">Unavailable / not recorded</Metadata><Metadata term="Review state">Unavailable / not recorded</Metadata>
          <Metadata term="Independent asset version">Unavailable / not recorded</Metadata><Metadata term="Visibility">Already authorized in this Design context</Metadata>
        </dl>
      </div>
