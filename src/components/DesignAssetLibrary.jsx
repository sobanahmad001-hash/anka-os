...
export default function DesignAssetLibrary({ workspace, contextKey, canUpload = false, canArchive = false, canSubmitReview = false, canDecideReview = false, currentUserId = '', busy = false, onUpload, onArchive, onReview, onRefresh, onClose, onFocusSource }) {
  ...
  return <section aria-labelledby="design-asset-library-title" className="rounded-2xl border border-violet-400/20 bg-slate-900/80 p-4 shadow-xl shadow-black/10 sm:p-5">
    ...
    <div className="mt-3 grid gap-3 sm:grid-cols-2">
      <label className="grid gap-1 text-xs font-semibold text-slate-400">Find existing media<input type="search" maxLength={240} value={state.filters.query} onChange={event => dispatch({ type: 'set_filter', name: 'query', value: event.target.value })} placeholder="Name, placement, usage notes or exact version ID" className={SELECT} /></label>
      <label className="grid gap-1 text-xs font-semibold text-slate-400">Latest version review state<select value={state.filters.review} onChange={event => dispatch({ type: 'set_filter', name: 'review', value: event.target.value })} className={SELECT}><option value="all">All review states</option><option value="approved">Approved</option><option value="draft">Draft</option><option value="unrecorded">Not recorded</option></select></label>
    </div>
    <p className="mt-2 text-xs text-slate-500">Find and inspect existing media before generating. Approval applies to the displayed exact version; it does not establish usage rights or permission for a different project.</p>
    <div aria-live="polite" className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs text-slate-400"><span>{visible.length} of {rows.length} authorized asset{rows.length === 1 ? '' : 's'}</span><span className="flex items-center gap-3">{(visible.length !== rows.length || state.filters.query || state.filters.review !== 'all') && <button type="button" onClick={() => dispatch({ type: 'reset_filters' })} className="font-semibold text-violet-300">Clear filters</button>}<button type="button" disabled={!canUpload} aria-expanded={uploadOpen} onClick={() => setUploadOpen(value => !value)} className="font-semibold text-cyan-300 focus:outline-none focus:ring-2 focus:ring-cyan-300/50 disabled:text-slate-600">{uploadOpen ? 'Hide upload' : 'Upload asset'}</button><button type="button" aria-expanded={comparisonOpen} onClick={() => setComparisonOpen(value => !value)} className="font-semibold text-cyan-300 focus:outline-none focus:ring-2 focus:ring-cyan-300/50">{comparisonOpen ? 'Hide output comparison' : 'Compare two outputs'}</button></span></div>
    ...
    {comparisonOpen && <DesignAssetComparison rows={rows.filter(row => row.mediaType === 'image')} contextKey={contextKey} accessOptions={accessOptions} onClose={() => setComparisonOpen(false)} onFocusSource={onFocusSource} />}
    ...
    {!rows.length ? <EmptyState title="No assets yet" text={canUpload ? 'Upload an authorized PNG draft or create an output from a Design direction.' : 'No authorized generated or uploaded assets are available.'} />
      : !visible.length ? <EmptyState title="No assets match these filters" text="Clear or change the filters; the underlying authorized results are unchanged." />
        : <div className={'mt-5 ' + (state.view === 'grid' ? 'grid gap-4 sm:grid-cols-2 xl:grid-cols-3' : 'space-y-3')}>{visible.map(row => <button type="button" key={row.id} onClick={() => dispatch({ type: 'select', assetId: row.id })} className={'w-full rounded-2xl border p-3 text-left focus:outline-none focus:ring-2 focus:ring-violet-400/60 ' + (state.selectedAssetId === row.id ? 'border-violet-400 bg-violet-500/10 ' : 'border-white/10 bg-slate-950/50 hover:border-violet-400/40 ') + (state.view === 'list' ? 'grid gap-3 sm:grid-cols-[7rem_1fr_auto] sm:items-center' : '')}>
          <div className={(state.view === 'list' ? 'h-20' : 'aspect-video') + ' overflow-hidden rounded-xl bg-black/30'}>{designAssetAccessState(row, accessOptions).canOpen ? (row.mediaType === 'video' ? <video src={row.previewUrl} preload="metadata" muted className="h-full w-full object-contain" /> : <img src={row.previewUrl} alt="" className="h-full w-full object-cover" />) : <div className="flex h-full items-center justify-center px-3 text-center text-xs text-slate-500">Preview {row.status === 'ready' ? 'link unavailable' : row.status}</div>}</div>
    ...
    {selected && <aside aria-labelledby="design-asset-detail-title" className="mt-6 rounded-2xl border border-white/10 bg-slate-950/70 p-4 sm:p-5">
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,1fr)]">
        <div className="overflow-hidden rounded-xl border border-white/10 bg-black/30">{access.canOpen ? (selected.mediaType === 'video' ? <video src={access.url} controls className="max-h-[32rem] w-full" /> : <img src={access.url} alt={selected.prompt || 'Selected Design output'} className="h-full max-h-[32rem] w-full object-contain" />) : <div className="flex min-h-56 items-center justify-center p-6 text-center text-sm text-slate-400">{access.message}</div>}</div>
        <dl className="grid content-start gap-3 text-sm sm:grid-cols-2">
          ...
          <Metadata term="Placement">{display(selected.placement)}</Metadata><Metadata term="Recorded rights or usage notes">{display(selected.rightsNotes, 'Not recorded — verify usage rights before reuse')}</Metadata>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        {access.canOpen ? <a href={access.url} target="_blank" rel="noreferrer" className="rounded-xl bg-violet-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-violet-500 focus:outline-none focus:ring-2 focus:ring-violet-300">Open or save signed {selected.mediaType === 'video' ? 'video' : 'image'}</a> : <span className="rounded-xl border border-white/10 px-4 py-2.5 text-sm font-semibold text-slate-500">{selected.mediaType === 'video' ? 'Video' : 'Image'} link unavailable</span>}
      ...
      {selected.mediaType === 'image' && !!selected.assetVersions?.length && <DesignAssetReviewPanel row={selected} reviews={(workspace.designAssetReviews || []).filter(item => item.asset_id === selected.assetId)} contextKey={contextKey} currentUserId={currentUserId} canSubmit={canSubmitReview} canDecide={canDecideReview} onReview={onReview} onRefresh={onRefresh} />}
    </aside>}
  </section>
}
