...
export default function MarketingCalendar({ organizationId, scopeRevision, signal, engagement, repository, onAccessError }) {
  ...
  return <section className="space-y-5">
    ...
    {view === 'month' ? <><MonthGrid days={days} firstOffset={firstOffset} entries={dated} />{!dated.length && <p role="status" className="rounded-2xl border border-dashed border-slate-700 p-8 text-center text-sm text-slate-500">No scheduled work matches this month and filters.</p>}</> : <CalendarList entries={dated} />}
