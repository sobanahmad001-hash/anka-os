...
function snapshot(brief, latest, sources, scopeKey) {
  const content = latest?.content || emptyCreativeBrief()
  const sourceIds = (sources || []).filter(source => source.creative_brief_version_id === latest?.id)
    .map(source => source.artifact_version_id)
  return { scopeKey, savedVersionId: latest?.id || '', savedContent: content,
    savedSourceIds: sourceIds, content, sourceIds }
}

function hasEdits(draft) {
  return JSON.stringify(draft.content) !== JSON.stringify(draft.savedContent)
    || JSON.stringify([...draft.sourceIds].sort()) !== JSON.stringify([...draft.savedSourceIds].sort())
}
...
const DesignCreativeBriefWorkspace = forwardRef(function DesignCreativeBriefWorkspace({
  ...
  useEffect(() => {
    ...
    setSaveError('')
    setDraft(current => current.scopeKey !== scopeKey
      || (current.savedVersionId !== (latest?.id || '') && !hasEdits(current))
      ? snapshot(brief, latest, workspace.creativeBriefSources, scopeKey) : current)
  }, [scopeKey, latest?.id]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  async function saveDraft() {
    ...
    if (!result) setSaveError('Save failed; your edits remain here. Retry the same draft or refresh its status.')
    return result
  }
  useImperativeHandle(ref, () => ({ isDirty: () => dirty, canSave: () => canSave && !conflict && Boolean(content.title.trim()), save: saveDraft }))
  function save(event) { event.preventDefault(); void saveDraft() }
  return <section aria-labelledby="creative-brief-title" className="rounded-2xl border border-violet-500/20 bg-slate-900/70 p-5">
    ...
    <form onSubmit={save} className="mt-5 grid gap-4 lg:grid-cols-2">
      ...
      <div className="flex flex-wrap gap-2 lg:col-span-2"><button disabled={!canSave || conflict || busy === 'save-brief' || !content.title.trim()} className={BUTTON}>{busy === 'save-brief' ? 'Saving…' : 'Save immutable draft version'}</button><button type="button" disabled={!canSave || dirty || conflict || !brief || !latest || !latest.validation_snapshot?.valid || busy === 'freeze-brief'} onClick={() => onFreeze({ creative_brief_id: brief.id, creative_brief_version_id: latest.id, expected_revision: brief.revision, operation_key: nextOperationKey() })} className="rounded-xl border border-white/10 px-4 py-2.5 text-sm font-semibold disabled:opacity-40">{busy === 'freeze-brief' ? 'Freezing…' : 'Use exact saved version for generation later'}</button></div>
    </form>
    {dirty && <p role="status" className="mt-4 text-xs text-amber-300">Unsaved brief edits · save this draft or explicitly discard before changing work.</p>}
    {conflict && <button type="button" className="mt-3 text-xs font-semibold text-violet-300 underline" onClick={() => { operation.current = null; setSaveError(''); setDraft(snapshot(brief, latest, workspace.creativeBriefSources, scopeKey)) }}>Discard edits and reload saved version</button>}
...
export default DesignCreativeBriefWorkspace
