...
export default function Sidebar({ collapsed = false }) {
  ...
  return (
    <aside id="workspace-sidebar" aria-label="Workspace sidebar" className={`shell-sidebar hidden min-h-0 w-60 shrink-0 flex-col overflow-y-auto border-r ${collapsed ? '' : 'md:flex'}`}>
      <div className="border-b shell-line px-5 py-5">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="truncate text-sm font-semibold shell-ink">{activeEnv.label}</div>
            <div className="mt-0.5 text-[11px] shell-muted">{activeEnv.description}</div>
          </div>
        </div>
        {activeEnv.key === 'sphere' && userDepartments.length > 0 && !allDepartments && (
          <div className="mt-4 flex flex-wrap gap-1">
            {userDepartments.map(userDept => <span key={userDept} className={`inline-flex rounded-full px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide ${departmentBadgeColors[userDept] || 'bg-gray-700 shell-ink'}`}>
              {userDept} department
            </span>)}
          </div>
        )}
        {activeEnv.key === 'sphere' && allDepartments && (
          <div className="mt-4">
            <span className="inline-flex rounded-full border border-violet-500/20 bg-violet-500/10 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide text-violet-300">
              All departments
            </span>
          </div>
        )}
      ...
      <nav aria-label="Workspace navigation" className="flex-1 p-3">
        <div className="space-y-1">
          {visibleItems.map((item) => {
            if (item.isHeader) {
              return (
                <div key={`header-${item.label}`} className="px-3 pb-1 pt-5">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.16em] shell-muted">
                    {item.label}
                  </p>
                </div>
              )
              ...
              <Link
                ...
                className={`group flex items-center gap-3 rounded-xl border px-3 py-2.5 text-[13px] font-medium transition-all ${
                    isCurrent
                      ...
                      : 'border-transparent shell-muted hover:shell-line hover:bg-white/[0.035] hover:shell-ink'
                  }`}
              >
      ...
      <div className="border-t shell-line p-4">
        <p className="text-[11px] leading-5 shell-muted">One system of record for delivery, review, and client visibility.</p>
      </div>
    </aside>
  )
}
...
function NavIcon({ path }) {
  const common = {
    ...
    'aria-hidden': true,
    className: 'shrink-0 shell-muted transition-colors group-hover:text-violet-300',
  }
