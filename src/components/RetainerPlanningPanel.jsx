...
export default function RetainerPlanningPanel({ project, engagement, services }) {
  ...
  }, [engagement.id, project.organization_id, services])

  useEffect(() => { load() }, [load])
  useEffect(() => { setPreviews({}) }, [month])
  ...
  async function previewMonth(plan) {
    ...
    try {
      ...
      setPreviews((current) => ({ ...current, [plan.id]: preview }))
    } catch (cause) {
  ...
  async function confirmPeriod(plan, period) {
    ...
    setBusy(`confirm-${plan.id}-${period.period_start}`)
    setError('')
    try {
  ...
  if (loading && !model) return <State>Loading retainer planning…</State>
  ...
  return (
    <section aria-label="Retainer planning" className="space-y-5">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <Metric title="Planned items" value={model.summary.plannedItems} />
      ...
      {model.plans.map((basePlan) => {
        const plan = previews[basePlan.id]
          ? applyRetainerMonthPreview(basePlan, previews[basePlan.id])
          : basePlan
  ...
  return (
    <article className="rounded-2xl border border-white/[0.07] bg-white/[0.025] p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex flex-wrap gap-2 text-[11px]"><Pill>{label(plan.status)}</Pill>{version && <Pill>{label(version.frequency)}</Pill>}{version && <Pill>{version.timezone}</Pill>}</div>
          <h3 className="mt-3 text-lg font-semibold">{version?.title || 'Unapproved recurring plan'}</h3>
          <p className="mt-1 text-sm text-slate-400">{version?.scope || 'No approved scope recorded.'}</p>
          <p className="mt-2 text-xs text-slate-500">{version ? `Effective ${date(version.effective_start)} to ${date(version.effective_end)} · v${version.version_number}` : 'No approved version'}</p>
        </div>
      ...
      <div className="mt-5 grid gap-4 xl:grid-cols-3">
        <Block title="Coverage">
          ...
          <p className="mt-3 text-xs text-slate-500">{plan.coverage.generated} generated canonical item{plan.coverage.generated === 1 ? '' : 's'} in this month.</p>
        </Block>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-2">
        <Block title="Approved commitment template">
          {!plan.templates.length && <p className="text-sm text-slate-500">No approved template items.</p>}
          <div className="space-y-2">{plan.templates.map((item) => <div key={item.id} className="flex flex-wrap justify-between gap-2 text-xs"><span className="text-slate-300">{item.title}</span><span className="text-slate-500">{item.default_assignee_id ? 'Assigned' : 'Unassigned'} · day +{item.start_offset_days} to +{item.due_offset_days}</span></div>)}</div>
        </Block>
      ...
      {plan.periods && <div className="mt-5 space-y-3">
        ...
        {plan.periods.map((period) => <div key={period.period_start} className="rounded-xl border border-white/[0.07] bg-black/10 p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <button type="button" onClick={() => confirm(period)} disabled={!period.eligible || busy}
