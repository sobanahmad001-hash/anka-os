...
const MAX_LENGTHS = Object.freeze({ term: 500, locale: 120, intent: 500, topic_group: 500, priority: 120, evidence_source: 1000, notes: 2000 })
...
export function KeywordArchitectureVersionSelect({ field, value, versions = [], inputClass, onChange }) {
  return <label className="block text-xs font-semibold uppercase tracking-[0.12em] text-slate-400">{field.label}<select aria-label={field.label} className={`${inputClass} mt-2 normal-case tracking-normal`} value={value || ''} onChange={event => onChange(event.target.value)}><option value="">Not needed for standalone request targets</option>{versions.map(version => <option key={version.id} value={version.id}>Version {version.version_number} · {new Date(version.created_at).toLocaleString()}</option>)}</select><span className="mt-2 block font-normal normal-case tracking-normal text-slate-500">Page targets are checked against this exact immutable version.</span></label>
}
  ...
  return <div>
    ...
    <div className="mt-3 space-y-4">{records.map((record, index) => <div key={index} className="rounded-2xl border border-slate-800 bg-slate-950/50 p-4">
      ...
      <div className="grid gap-4 md:grid-cols-2">{field.recordFields.map(([key, label, kind, options]) => {
        ...
        const ariaLabel = `${label} for keyword ${index + 1}`
        return <label key={key} className={`text-xs font-semibold uppercase tracking-[0.12em] text-slate-500 ${wide ? 'md:col-span-2' : ''}`}>{label}{measured && (record[key] === '' || record[key] === null || record[key] === undefined) && <span className="ml-2 font-normal normal-case tracking-normal text-slate-600">Not available</span>}{kind === 'textarea_optional' ? <textarea aria-label={ariaLabel} maxLength={MAX_LENGTHS[key]} rows="3" className={`${inputClass} mt-2 normal-case tracking-normal`} value={record[key] || ''} onChange={event => setValue(event.target.value)} /> : kind === 'select' || kind === 'keyword_target' ? <select aria-label={ariaLabel} required={required} className={`${inputClass} mt-2 normal-case tracking-normal`} value={record[key] || ''} onChange={event => setValue(event.target.value)}><option value="">{kind === 'keyword_target' ? 'Select existing target' : 'Select one'}</option>{choices.map(choice => { const value = typeof choice === 'object' ? choice.value : choice; const choiceLabel = typeof choice === 'object' ? choice.label : choice; return <option key={value} value={value}>{choiceLabel}</option> })}</select> : <input aria-label={ariaLabel} maxLength={MAX_LENGTHS[key]} required={required} type={inputType} min={kind === 'number_optional' ? '0' : undefined} step={key === 'search_volume' ? '1' : kind === 'number_optional' ? 'any' : undefined} className={`${inputClass} mt-2 normal-case tracking-normal`} value={record[key] ?? ''} onChange={event => setValue(event.target.value)} />}</label>
      })}</div>
