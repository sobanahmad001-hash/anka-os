...
function VersionCard({ row, version, accessOptions, label }) {
  ...
  return <article className="min-w-0 rounded-xl border border-white/10 bg-slate-950/60 p-3">
    ...
    <div className="mt-3 flex min-h-40 items-center justify-center overflow-hidden rounded-lg bg-black/30">{access.canOpen ? (row.mediaType === 'video' ? <video src={access.url} controls className="max-h-72 w-full" /> : <img src={access.url} alt={`${row.recorded.name || 'Design asset'} version ${version.version_number}`} className="max-h-72 w-full object-contain" />) : <p className="p-4 text-center text-xs text-amber-200">{access.message}</p>}</div>
...
export default function DesignAssetVersionBrowser({ row, contextKey, accessOptions }) {
  ...
  return <section aria-labelledby="asset-version-history-title" className="mt-5 rounded-xl border border-cyan-400/20 bg-cyan-500/[0.03] p-4">
    ...
    {row.mediaType === 'video' && versions.length === 1 ? <div className="mt-4"><VersionCard row={row} version={versions[0]} accessOptions={accessOptions} label="Exact saved draft" /><p className="mt-2 text-xs text-slate-400">This is a saved copy, not an approved output. Video replacement and editing are not part of this operation.</p></div> : versions.length < 2 ? <p className="mt-4 rounded-lg border border-dashed border-white/10 p-4 text-sm text-slate-500">Upload a replacement to create a second immutable draft version for comparison.</p> : <>
