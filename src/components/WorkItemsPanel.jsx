...
export default function WorkItemsPanel({ workspace, owners, organizationId, signal, onRefresh }) {
  ...
  const authority = useAssignmentCapabilities(organizationId, workspace.engagement.project_id, items)
  const [dependencies, setDependencies] = useState([])
  const [view, setView] = useState('list')
  const [filters, setFilters] = useState({ status: '', assignee: '', department: '', priority: '', due: '', pagePath: '' })
  ...
  const visibleItems = useMemo(() => filterAndSortWorkItems(items, filters, sort).map(item => ({
    ...item, assignmentCapabilities: recordAssignmentCapabilities(authority.data, 'engagement_work_items', item),
  })), [items, filters, sort, authority.data])
  const pagePathOptions = useMemo(() => [...new Set(items.map(item => item.linked_page_path).filter(Boolean))]
    .sort((left, right) => left.localeCompare(right))
    .map(path => ({ value: path, label: path })), [items])
  ...
  function openNew(parentWorkItemId = '') {
    ...
    setEditor({ ...EMPTY_WORK_ITEM, engagement_id: workspace.engagement.id, parent_work_item_id: parentWorkItemId })
  }
  ...
  async function save(event) {
    ...
    if (!editorAuthority.can_execute) return
  ...
  async function moveWorkItem(workItemId, targetStatus, beforeWorkItemId = null) {
    ...
    if (!item || !recordAssignmentCapabilities(authority.data, 'engagement_work_items', item).can_execute) return
  ...
  const editorAuthority = editor?.id
    ? recordAssignmentCapabilities(authority.data, 'engagement_work_items', editor)
    : { can_assign: authority.data?.can_assign === true || authority.data?.assignable_departments?.includes(editor?.department_id),
      can_execute: authority.data?.can_create_unassigned === true }
  ...
  return <section className="mt-7 space-y-5">
    ...
    {view !== 'automations' && <div className="grid gap-3 rounded-2xl border border-white/[0.07] bg-[#0e111a]/80 p-4 sm:grid-cols-2 xl:grid-cols-6">
      ...
      <Filter label="Content page" value={filters.pagePath} onChange={pagePath => setFilters({ ...filters, pagePath })} options={pagePathOptions} />
    </div>}
    ...
    {editor && <div className="fixed inset-0 z-50 flex justify-end bg-black/75"><form onSubmit={save} className="h-full w-full max-w-2xl overflow-y-auto border-l border-white/10 bg-[#111520] p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-violet-400">Work item detail</p><div className="mt-1 flex items-center gap-2"><h2 className="text-xl font-semibold">{editor.id ? 'Edit work item' : 'Create work item'}</h2>{editor.automation_flagged_at && <AutomationBadge />}</div></div><button type="button" onClick={() => setEditor(null)} className="text-sm text-slate-500 hover:text-white">Close</button></div>
      ...
      <div className="mt-7 flex items-center justify-between gap-3">{editor.id ? <button type="button" disabled={saving || !editorAuthority.can_execute} onClick={remove} className="rounded-xl border border-red-800/60 px-4 py-2.5 text-sm font-semibold text-red-300 disabled:opacity-50">Remove from work list</button> : <span />}<button disabled={saving || !editorAuthority.can_execute || !editor.title.trim()} className="rounded-xl bg-violet-500 px-5 py-2.5 text-sm font-semibold disabled:opacity-40">{saving ? 'Saving…' : 'Save work item'}</button></div>
    </form></div>}
  </section>
}

function WorkItemsBoard({ columns, relations, owners, artifacts, loading, moving, draggedItemId, onDragStart, onMove, onMoveWithinColumn, onOpen }) {
  ...
  return <div className="overflow-x-auto pb-2"><div className="grid min-w-[1180px] grid-cols-4 gap-4">
    {WORK_ITEM_BOARD_COLUMNS.map(column => <section key={column.value} aria-labelledby={`work-column-${column.value}`} onDragOver={event => event.preventDefault()} onDrop={() => draggedItemId && onMove(draggedItemId, column.value)} className="min-h-[420px] rounded-2xl border border-white/[0.07] bg-[#0e111a]/80 p-3">
      ...
      <div className="mt-2 space-y-3">{columns[column.value].map((item, index) => <WorkItemCard key={item.id} item={item} index={index} columnLength={columns[column.value].length} openSubtasks={relations.openSubtaskCount(item.id)} unresolvedDependencies={relations.unresolvedDependencyCount(item.id)} owners={owners} artifact={artifacts.find(candidate => candidate.id === item.linked_artifact_id)} moving={moving} onDragStart={onDragStart} onMove={onMove} onMoveWithinColumn={onMoveWithinColumn} onOpen={onOpen} />)}</div>
...
function WorkItemCard({ item, index, columnLength, openSubtasks, unresolvedDependencies, owners, artifact, moving, onDragStart, onMove, onMoveWithinColumn, onOpen }) {
  moving = moving || !item.assignmentCapabilities?.can_execute
  ...
  return <article draggable={!moving} onDragStart={event => { event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('text/plain', item.id); onDragStart(item.id) }} onDragEnd={() => onDragStart('')} onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); event.stopPropagation(); const draggedId = event.dataTransfer.getData('text/plain'); if (draggedId && draggedId !== item.id) onMove(draggedId, item.status, item.id) }} className="rounded-xl border border-white/[0.08] bg-[#151a27] p-4 shadow-lg shadow-black/10 transition hover:border-violet-500/25">
    ...
    {item.linked_page_path && <button type="button" onClick={() => onOpen(item)} className="ml-2 mt-3 inline-flex rounded-full bg-amber-500/10 px-2.5 py-1 text-[10px] font-semibold text-amber-300">Page · {item.linked_page_path}</button>}
    ...
    {(item.parent_work_item_id || openSubtasks > 0 || unresolvedDependencies > 0) && <div className="mt-3 flex flex-wrap gap-2">{item.parent_work_item_id && <span className="rounded-full bg-white/[0.05] px-2.5 py-1 text-[10px] font-semibold text-slate-400">Subtask</span>}{openSubtasks > 0 && <span className="rounded-full bg-blue-500/10 px-2.5 py-1 text-[10px] font-semibold text-blue-300">{openSubtasks} open subtask{openSubtasks === 1 ? '' : 's'}</span>}{unresolvedDependencies > 0 && <span className="rounded-full bg-amber-500/10 px-2.5 py-1 text-[10px] font-semibold text-amber-300">{unresolvedDependencies} blocker{unresolvedDependencies === 1 ? '' : 's'}</span>}</div>}
...
function RelationPanel({ title, action, children }) { return <section className="rounded-xl border border-white/[0.07] bg-black/10 p-4"><div className="flex items-center justify-between gap-3"><h3 className="text-sm font-semibold">{title}</h3>{action}</div><div className="mt-3 space-y-2">{children}</div></section> }
function RelationRow({ item, onOpen, onRemove }) { return <div className="flex items-center gap-2 rounded-lg border border-white/[0.06] bg-white/[0.025] p-2.5"><span aria-label={item.status === 'done' ? 'Complete' : 'Open'} className={`h-2 w-2 shrink-0 rounded-full ${item.status === 'done' ? 'bg-emerald-400' : 'bg-slate-500'}`} /><button type="button" onClick={onOpen} className="min-w-0 flex-1 truncate text-left text-xs text-slate-300 hover:text-white">{item.title}</button>{onRemove && <button type="button" onClick={onRemove} className="text-[10px] font-semibold text-red-300">Remove</button>}</div> }
function RelationEmpty({ text }) { return <p className="py-2 text-xs text-slate-600">{text}</p> }
...
function AutomationBadge({ compact = false }) { return <span title="Automation requested this assignee's attention" className="inline-flex shrink-0 items-center rounded-full bg-fuchsia-500/15 px-2 py-0.5 text-[10px] font-semibold text-fuchsia-300">{compact ? 'Auto' : 'Automation'}</span> }
