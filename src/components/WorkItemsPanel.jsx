...
export default function WorkItemsPanel({ workspace, owners, onRefresh }) {
  ...
  return <section className="mt-7 space-y-5">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-xl font-semibold">Work</h2><p className="mt-1 text-sm text-slate-500">One filtered engagement queue, available as a list, board, calendar, or timeline.</p></div><div className="flex items-center gap-3"><div aria-label="Work view" className="flex flex-wrap rounded-xl border border-white/10 bg-black/20 p-1">{[['list', 'List'], ['board', 'Board'], ['calendar', 'Calendar'], ['timeline', 'Timeline']].map(([value, label]) => <button key={value} type="button" aria-pressed={view === value} onClick={() => setView(value)} className={`rounded-lg px-3 py-2 text-xs font-semibold ${view === value ? 'bg-white/10 text-white' : 'text-slate-500 hover:text-white'}`}>{label}</button>)}</div><button onClick={() => openNew()} className="rounded-xl bg-violet-500 px-4 py-2.5 text-sm font-semibold">New work item</button></div></div>
    ...
    {view === 'list' ? <div className="overflow-x-auto rounded-2xl border border-white/[0.07] bg-[#0e111a]/80">
      ...
      </tr></thead><tbody>{visibleItems.map(item => <tr key={item.id} onClick={() => { setDependencyCandidate(''); setEditor({ ...item }) }} className="cursor-pointer border-t border-white/[0.06] hover:bg-white/[0.025]"><td className="px-4 py-3"><p className="font-medium text-white">{item.title}</p><p className="mt-1 line-clamp-1 text-xs text-slate-600">{item.parent_work_item_id ? 'Subtask · ' : ''}{labelize(item.work_item_type)}{item.description ? ` · ${item.description}` : ''}</p></td><td className="px-4 py-3"><Status value={item.status} /></td><td className="px-4 py-3 text-slate-300">{labelize(item.priority)}</td><td className="px-4 py-3 text-slate-400">{ownerLabel(owners, item.assignee_id)}</td><td className="px-4 py-3 text-slate-400">{labelize(item.department_id) || 'Shared'}</td><td className="px-4 py-3 text-slate-400">{item.due_date || 'Not set'}</td></tr>)}</tbody></table>
    ...
    </div> : view === 'board' ? <WorkItemsBoard columns={boardColumns} relations={relations} owners={owners} artifacts={workspace.workItemArtifacts || []} loading={loading} moving={moving} draggedItemId={draggedItemId} onDragStart={setDraggedItemId} onMove={moveWorkItem} onMoveWithinColumn={moveWithinColumn} onOpen={item => { setDependencyCandidate(''); setEditor({ ...item }) }} /> : view === 'calendar' ? <WorkCalendarView items={visibleItems} loading={loading} onOpen={item => { setDependencyCandidate(''); setEditor({ ...item }) }} /> : <WorkTimelineView items={visibleItems} dependencies={dependencies} loading={loading} onOpen={item => { setDependencyCandidate(''); setEditor({ ...item }) }} />}
