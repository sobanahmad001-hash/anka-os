...
function RecordEditor({ record, workspace, capability, disabled, save }) {
  ...
  return <div className="mt-3 grid gap-2 border-t border-white/[0.06] pt-3 sm:grid-cols-2">
    ...
    {!isTask && <label className="text-[10px] text-slate-500">Handoff department<select className={`${INPUT} mt-1 w-full`} value={departmentId} disabled={disabled || !capability.can_assign} onChange={event => setDepartmentId(event.target.value)}><option value="">Unassigned</option>{departments.map(item => <option key={item.department_id} value={item.department_id}>{label(item.department_id)}</option>)}</select></label>}
