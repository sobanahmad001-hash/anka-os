...
export default function GeneralContentRequestsPanel({ repository }) {
  ...
  async function submit(event) {
    ...
    try {
      ...
      setMessage(form.format === 'article' ? 'Article request saved as a writing brief.' : form.output_path === 'internal_engine'
  ...
  return <div className="grid gap-6 xl:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]">
    <form onSubmit={submit} className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6">
      ...
      <h2 className="mt-1 text-2xl font-semibold">Make a post, reel, or article</h2>
      ...
      <div className="mt-6 space-y-5">
        ...
        <label className="block text-xs font-semibold uppercase tracking-[0.12em] text-slate-400">Format<select className={`${INPUT} mt-2 normal-case tracking-normal`} value={form.format} onChange={event => setForm(current => ({ ...current, format: event.target.value, output_path: event.target.value === 'article' ? 'internal_engine' : current.output_path }))}>{CONTENT_REQUEST_FORMATS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
        {form.format !== 'article' && <label className="block text-xs font-semibold uppercase tracking-[0.12em] text-slate-400">Output path<select className={`${INPUT} mt-2 normal-case tracking-normal`} value={form.output_path} onChange={event => setForm(current => ({ ...current, output_path: event.target.value }))}>{CONTENT_REQUEST_OUTPUT_PATHS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>}
        <p className="rounded-xl border border-slate-800 bg-slate-950/60 p-3 text-xs leading-5 text-slate-400">{form.format === 'article' ? 'Article requests save a writing brief without media or Figma output.' : 'Internal generation remains project-only. CP3 owns Figma reference-page generation and stores the authenticated page URL on the request.'}</p>
    ...
    <section className="space-y-4">
      ...
      {loading ? <div className="rounded-2xl border border-slate-800 p-12 text-center text-sm text-slate-500">Loading general requests…</div> : workspace.requests.length ? workspace.requests.map(request => { const handoff = workspace.handoffs.find(item => item.content_request_id === request.id); return <article key={request.id} className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-amber-400">{request.format.replaceAll('_', ' ')}</p><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-300">{request.brief}</p></div><span className="rounded-full bg-slate-950 px-2.5 py-1 text-[10px] font-semibold uppercase text-slate-300">{request.status.replaceAll('_', ' ')}</span></div><div className="mt-4 flex flex-wrap gap-2 text-[11px] text-slate-500"><span>{brandNames.get(request.brand_id) || 'No brand'}</span><span>·</span><span>{request.format === 'article' ? 'Writing brief' : request.output_path === 'internal_engine' ? 'Anka OS request' : 'Figma handoff'}</span><span>·</span><span>{new Date(request.created_at).toLocaleString()}</span></div>{handoff && <a href={handoff.figma_handoff_url} className="mt-4 inline-block text-sm font-semibold text-amber-300 hover:text-amber-200">Open authenticated Figma reference →</a>}<ContentRequestReviewPanels request={request} /><GeneralRequestQuickTaskCopy request={request} /></article> }) : <div className="rounded-2xl border border-dashed border-slate-700 p-12 text-center text-sm text-slate-500">No general content requests yet.</div>}
    </section>
  </div>
}
