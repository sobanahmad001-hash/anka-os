...
export default function DesignWorkshop() {
  ...
  const [handoffUncertainTarget, setHandoffUncertainTarget] = useState('')
  ...
  useEffect(() => {
    ...
    setEngagements([]); setWorkspace(null); setModal(null); setPendingSelection(null); setHandoffUncertainTarget(''); setEngagementLoadState(studio ? 'loading' : 'error'); setWorkspaceLoadState('idle'); setBusy(studio ? 'load' : ''); setError('')
  ...
  useEffect(() => {
    if (engagementId) refresh().then(refreshed => { if (refreshed) setHandoffUncertainTarget('') })
  ...
  async function prepareHandoff(release) {
    ...
    const targetKey = productionHandoffContextKey(release)
  ...
  async function refreshHandoffStatus(release) {
    ...
    if (refreshed) setHandoffUncertainTarget(current => current === targetKey ? '' : current)
    return refreshed
  }
...
function WorkshopWorkspace({ workspace, assetLibraryContextKey, focusedSessionId, focusedVersionId: navigationFocusedVersionId, focusedDraftId, canUploadAsset, onUploadAsset, canArchiveAsset, onArchiveAsset, canPromoteExperiment, onCreateFlow, onCreate, onGenerate, onGenerateImage, onRefreshImageJob, onRetryImageJob, onGenerateVariants, onGenerateVideo, onGeneratePage, onSubmitPage, onApprovePage, onExportPage, onDownloadExport, canPrepareHandoff, handoffUncertainTarget, onPrepareHandoff, onDownloadHandoff, onRefreshHandoffs, onRefine, onPromote, onSetWorking, onSelect, onRelease, busy }) {
  ...
  return <div className="mt-6 space-y-6">
    ...
    {release && <ProductionHandoffPanel key={productionHandoffContextKey(release)} release={release} packages={workspace.handoffPackages} directionVersions={workspace.directionVersions} mediaAssets={workspace.mediaAssets} variants={workspace.variants} mediaAccess={{ issuedAt: workspace.mediaUrlsRequestedAt, expiresInSeconds: workspace.mediaUrlExpiresIn, trustedOrigin: workspace.mediaUrlOrigin }} canPrepare={canPrepareHandoff} createUncertain={handoffUncertainTarget === productionHandoffContextKey(release)} busy={busy} onPrepare={onPrepareHandoff} onDownload={onDownloadHandoff} onRefresh={() => onRefreshHandoffs(release)} />}
