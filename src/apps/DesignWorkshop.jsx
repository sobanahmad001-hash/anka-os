...
function RefineModal({ version, onClose, onSave, busy }) {
  const [content, setContent] = useState(version.content)
  const set = (key, value) => setContent(current => ({ ...current, [key]: value }))
  return <Modal title={`Refine “${version.content.title}” as a new version`} onClose={onClose}><form onSubmit={event => { event.preventDefault(); onSave(content) }} className="space-y-4"><Field label="Concept title"><input required className={INPUT} value={content.title || ''} onChange={event => set('title', event.target.value)} /></Field><Field label="Rationale"><textarea required rows="4" className={INPUT} value={content.rationale || ''} onChange={event => set('rationale', event.target.value)} /></Field><Field label="Creative thesis"><textarea required rows="3" className={INPUT} value={content.creative_thesis || ''} onChange={event => set('creative_thesis', event.target.value)} /></Field><Field label="Imagery direction"><textarea rows="3" className={INPUT} value={content.imagery_direction || ''} onChange={event => set('imagery_direction', event.target.value)} /></Field><Field label="Layout direction"><textarea rows="3" className={INPUT} value={content.layout_direction || ''} onChange={event => set('layout_direction', event.target.value)} /></Field><button disabled={busy === 'refine'} className={`${BUTTON} w-full`}>Create linked version</button></form></Modal>
}
