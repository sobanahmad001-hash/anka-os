...
import DesignPackageReviewPanel from '../components/DesignPackageReviewPanel.jsx'
...
export default function DesignWorkshop() {
  ...
  useEffect(() => {
    ...
    setEngagements([]); setWorkspace(null); setBriefDirty(false); setModal(null); setPendingSelection(null); setHandoffUncertainTargets(new Set()); setEngagementLoadState(studio ? 'loading' : 'error'); setWorkspaceLoadState('idle'); setBusy(studio ? 'load' : ''); setError('')
  ...
  async function act(key, action, capability = 'createDraft') {
    ...
    try {
      ...
      if (deferredContext.current) {
        ...
        deferredContext.current = null
        setPendingSelection(null); setBriefDirty(false); setWorkspace(null); setSearchParams(next)
      } else await refresh()
  ...
  function requestSelection(params) {
    if (modal || briefRef.current?.isDirty()) setPendingSelection({ params })
    else { setBriefDirty(false); setWorkspace(null); setSearchParams(params) }
  }
  function saveCurrentAndSwitch() {
    ...
    if (briefRef.current?.isDirty()) {
      setSwitchSaving(true)
      void briefRef.current.save().catch(capture).finally(() => setSwitchSaving(false))
      return
    }
  ...
  function discardAndSwitch() {
    ...
    deferredContext.current = null; setPendingSelection(null); setModal(null); setBriefDirty(false); setWorkspace(null); setSearchParams(next)
  }
  ...
  async function prepareHandoff(release, designPackageVersionId = null) {
    ...
    try {
      await productionHandoffs.create(release.id, engagementId, designPackageVersionId)
  ...
  async function releaseHandoffToClient(packageId, requestId, summary) {
    if (!officialReady || !['system_owner', 'operations_admin', 'executive'].includes(activeMembership?.role)) {
      throw new Error('Current organization release authority is required.')
    }
    setBusy(`client-release-${packageId}`); setError('')
    try {
      const result = await productionHandoffs.releaseToClient(activeOrganizationId, packageId, requestId, summary)
      if (!await refresh()) throw new Error('Release completed, but the refreshed Design view could not be confirmed. Refresh before retrying.')
      return result
  ...
  return <Shell>
    ...
    <ContextStrip context={context} navigation={navigationContext} organizationName={activeOrganization?.name} unsaved={Boolean(modal) || briefDirty} capabilities={capabilities} officialReady={officialReady} />
    ...
    {workspace && <div className={tab === 'workshop' ? 'mt-6' : 'hidden'}><DesignCreativeBriefWorkspace ref={briefRef} workspace={workspace}
      activeServiceId={context.service?.id || workspace.designServices[0]?.id || ''} workRecord={workspace.navigationWorkRecord}
      busy={busy} canSave={officialReady && workspaceLoadState === 'ready' && capabilities.createDraft}
      onDirtyChange={setBriefDirty} onSave={input => act('save-brief', () => studio.saveCreativeBrief(input))}
      onFreeze={input => act('freeze-brief', () => studio.freezeCreativeBrief(input))} /></div>}
    {!workspace ? <div className="py-20 text-center text-sm text-slate-500">Loading exact versions…</div>
    ...
    {pendingSelection && <ContextSwitchDialog saving={switchSaving} canSave={context.accessValid && officialReady && capabilities.createDraft
      && !(modal && briefDirty) && (!briefDirty || briefRef.current?.canSave())} onStay={() => setPendingSelection(null)} onSave={saveCurrentAndSwitch} onDiscard={discardAndSwitch} />}
...
function ContextSwitchDialog({ canSave, saving, onStay, onSave, onDiscard }) {
  return <div role="dialog" aria-modal="true" aria-labelledby="design-context-switch-title" className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 p-4"><section className="w-full max-w-lg rounded-2xl border border-white/10 bg-slate-900 p-6 shadow-2xl"><h2 id="design-context-switch-title" className="text-xl font-semibold">You have unsaved changes</h2><p className="mt-2 text-sm leading-6 text-slate-400">Choose what happens before Design changes organization, project, engagement, brand, service, or work identity. Attachments and pending confirmations never carry into the next context.</p><div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" onClick={onStay} disabled={saving} className="rounded-xl border border-white/10 px-4 py-2.5 text-sm font-semibold">Stay</button><button type="button" disabled={!canSave || saving} title={canSave ? '' : 'Save is unavailable because current access or official context is no longer valid.'} onClick={onSave} className={`${BUTTON} bg-slate-700`}>Save to current context</button><button type="button" onClick={onDiscard} disabled={saving} className="rounded-xl border border-red-500/30 px-4 py-2.5 text-sm font-semibold text-red-200">Discard</button></div></section></div>
}
...
function WorkshopWorkspace({ workspace, assetLibraryContextKey, focusedSessionId, focusedVersionId: navigationFocusedVersionId, focusedDraftId, canUploadAsset, onUploadAsset, canArchiveAsset, onArchiveAsset, onRefreshAssetLinks, currentUserId, canSubmitAssetReview, canDecideAssetReview, onAssetReview, canPromoteExperiment, onCreateFlow, onCreate, onGenerate, onGenerateImage, onRefreshImageJob, onRetryImageJob, onGenerateVariants, onGenerateVideo, onGeneratePage, onSubmitPage, onApprovePage, onExportPage, onDownloadExport, canPrepareHandoff, canReleaseHandoffToClient, onReleaseHandoffToClient, handoffUncertainTargets, handoffContext, onPrepareHandoff, onDownloadHandoff, onRefreshHandoffs, onRefine, onPromote, onSetWorking, onSelect, onRelease, busy }) {
  ...
  return <div className="mt-6 space-y-6">
    ...
    {assetLibraryOpen && <DesignAssetLibrary key={contextKey} contextKey={contextKey} workspace={workspace} canUpload={canUploadAsset} canArchive={canArchiveAsset} busy={busy === 'asset-upload' || busy.startsWith('asset-archive-')} onUpload={onUploadAsset} onArchive={onArchiveAsset} onReview={onAssetReview} currentUserId={currentUserId} canSubmitReview={canSubmitAssetReview} canDecideReview={canDecideAssetReview} onRefresh={onRefreshAssetLinks} onClose={() => setAssetLibraryOpen(false)} onFocusSource={focusAssetSource} />}
    ...
    {release && <ProductionHandoffPanel key={productionHandoffContextKey(release, handoffContext)} release={release} packages={workspace.handoffPackages} clientReleases={workspace.clientHandoffReleases} canReleaseClient={canReleaseHandoffToClient} onReleaseClient={onReleaseHandoffToClient} packageOptions={workspace.deliveryPackageVersions.filter(version => workspace.approvals.some(approval => approval.artifact_version_id === version.id && approval.decision === 'approved') && workspace.deliveryPackageArtifacts.some(artifact => artifact.id === version.artifact_id)).map(version => ({ ...version, title: workspace.deliveryPackageArtifacts.find(artifact => artifact.id === version.artifact_id)?.title || '' }))} directionVersions={workspace.directionVersions} mediaAssets={workspace.mediaAssets} variants={workspace.variants} mediaAccess={{ issuedAt: workspace.mediaUrlsRequestedAt, expiresInSeconds: workspace.mediaUrlExpiresIn, trustedOrigin: workspace.mediaUrlOrigin }} canPrepare={canPrepareHandoff} createUncertain={handoffUncertainTargets.has(productionHandoffContextKey(release, handoffContext))} busy={busy} onPrepare={onPrepareHandoff} onDownload={onDownloadHandoff} onRefresh={() => onRefreshHandoffs(release)} />}
