...
import { keywordDuplicateCounts } from '../data/marketingKeywordResearch.js'
...
export default function TechnicalSeoTracking() {
  ...
  const [keywordDraft, setKeywordDraft] = useState({ keyword: '', targetRankTier: '', sourceArtifactId: '' })
  ...
  async function saveKeyword(event) {
    event.preventDefault()
    const result = await perform(() => technicalSeo.saveKeyword({ ...keywordDraft, pageId }), 'Keyword added to rank tracking.')
    if (result) { setKeywordDraft({ keyword: '', targetRankTier: '', sourceArtifactId: '' }); await loadKeywords(pageId) }
  }

  async function fetchKeywordRanks() {
    const result = await perform(() => technicalSeo.fetchKeywordRanks(pageId), 'Search Console rank snapshots saved for today.')
  ...
  async function setKeywordActive(keyword) {
    setBusy(true); setError(''); setMessage('')
    try {
      const updated = await technicalSeo.setKeywordActive(keyword.id, !keyword.active)
      await loadKeywords(pageId)
