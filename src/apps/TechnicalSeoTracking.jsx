...
import { INDEX_STATUSES, TRACKED_PAGE_TYPES, filterHealth, healthSummary, labelize, pageDepth } from '../data/technicalSeo.js'
...
export default function TechnicalSeoTracking() {
  ...
  const [filters, setFilters] = useState({ pageType: '', indexStatus: '', attention: '' })
  ...
  return <div className="h-full overflow-y-auto bg-slate-950 text-white">
    ...
    <main className="mx-auto max-w-7xl space-y-5 p-6">
      ...
      {selected && <div className="grid gap-5 xl:grid-cols-[1fr_1fr]"><Panel title="Audit history" description={selected.page_url}><div className="mb-4 flex flex-wrap gap-2"><button disabled={busy} onClick={() => perform(() => technicalSeo.inspectPage(pageId), 'Search Console inspection saved as today’s snapshot.')} className={PRIMARY}>Inspect with Search Console</button><span className="self-center text-xs text-slate-500">Read-only URL Inspection API; manual Core Web Vitals remain separate.</span></div><div className="space-y-2">{audits.map(audit => <div key={audit.id} className="rounded-xl border border-slate-800 bg-slate-950/50 p-3"><div className="flex justify-between gap-3"><p className="text-sm font-medium">{audit.audit_date} · {labelize(audit.index_status || 'unknown')}</p><span className="text-xs text-slate-500">{labelize(audit.source_type)}</span></div><p className="mt-2 text-xs text-slate-400">Mobile {audit.core_web_vitals_mobile ?? '—'} · Desktop {audit.core_web_vitals_desktop ?? '—'} · Schema {audit.schema_valid === null ? 'unknown' : audit.schema_valid ? 'valid' : 'invalid'}</p><p className="mt-1 text-xs text-amber-300">{(audit.issues || []).join(' · ') || 'No manual issues recorded'}</p>{audit.notes && <p className="mt-2 text-xs leading-5 text-slate-500">{audit.notes}</p>}</div>)}{!audits.length && <p className="text-sm text-slate-500">No audits recorded.</p>}</div></Panel>
