...
function EngagementWorkspace({ workspace, owners, organizationId, membership, signal, initialTab = 'overview', returnProjectId = '', onRefresh, onBack }) {
  ...
  return <div className="h-full overflow-y-auto text-white"><div className="mx-auto max-w-[1480px] px-4 py-6 sm:px-6 lg:px-8">
    ...
    {tab === 'pipeline' ? <PipelineWorkspace workspace={workspace} organizationId={organizationId} membership={membership} signal={signal} /> : tab === 'work' ? <WorkItemsPanel workspace={workspace} owners={owners} organizationId={organizationId} signal={signal} onRefresh={onRefresh} /> : tab === 'development' && hasDevelopment ? <DevelopmentTrackingPanel workspace={workspace} onRefresh={onRefresh} /> : <div className="mt-7 grid gap-6 xl:grid-cols-[1.4fr_1fr]">
...
function PipelineWorkspace({ workspace, organizationId, membership, signal }) {
