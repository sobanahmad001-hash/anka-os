...
const EMPTY_EVENT = { eventName: '', category: 'conference', venue: '', location: '', startDate: '', endDate: '', sourceUrl: '' }
  ...
  return <div className="h-full overflow-y-auto bg-slate-950 text-white">
    ...
    <main className="mx-auto max-w-7xl space-y-5 p-6">
      ...
      <div className="grid gap-5 xl:grid-cols-[1fr_1.45fr]">
        <Panel title="Add external event" description="Events belong to the selected brand."><form className="grid gap-3" onSubmit={createEvent}><input className={INPUT} placeholder="Event name" maxLength={200} required value={eventDraft.eventName} onChange={e => setEventDraft({ ...eventDraft, eventName: e.target.value })}/><select className={INPUT} value={eventDraft.category} onChange={e => setEventDraft({ ...eventDraft, category: e.target.value })}>{EVENT_CATEGORIES.map(value => <option key={value} value={value}>{labelize(value)}</option>)}</select><div className="grid grid-cols-2 gap-3"><input type="date" className={INPUT} required value={eventDraft.startDate} onChange={e => setEventDraft({ ...eventDraft, startDate: e.target.value })}/><input type="date" className={INPUT} min={eventDraft.startDate} value={eventDraft.endDate} onChange={e => setEventDraft({ ...eventDraft, endDate: e.target.value })}/></div><input className={INPUT} placeholder="Venue (optional)" value={eventDraft.venue} onChange={e => setEventDraft({ ...eventDraft, venue: e.target.value })}/><input className={INPUT} placeholder="Location (optional)" value={eventDraft.location} onChange={e => setEventDraft({ ...eventDraft, location: e.target.value })}/><input type="url" className={INPUT} placeholder="Source URL (optional)" value={eventDraft.sourceUrl} onChange={e => setEventDraft({ ...eventDraft, sourceUrl: e.target.value })}/><button className={BUTTON} disabled={busy || !brandId}>Add event</button></form></Panel>

        <Panel title={selectedEvent?.event_name || 'Event content plan'} description={selectedEvent ? `${selectedEvent.start_date} · ${labelize(selectedEvent.category)}` : 'Select an event to plan content.'}>
          {selectedEvent && <><div className="grid gap-2 md:grid-cols-2">{links.map(link => <div key={link.id} className="rounded-xl border border-slate-800 bg-slate-950/50 p-3"><div className="flex items-center justify-between gap-2"><span className="rounded-full bg-violet-950 px-2 py-1 text-xs text-violet-300">{labelize(link.content_type)}</span><select aria-label={`Status for ${labelize(link.content_type)}`} disabled={busy} className="rounded-lg border border-slate-700 bg-slate-900 px-2 py-1 text-xs text-slate-300" value={link.status} onChange={e => updateLinkStatus(link, e.target.value)}>{EVENT_LINK_STATUSES.map(value => <option key={value} value={value}>{labelize(value)}</option>)}</select></div><p className="mt-2 text-sm">{displayWorkItem(link)}</p><p className="mt-1 text-xs text-slate-500">Lead time {link.lead_time_days} days</p></div>)}{!links.length && <p className="text-sm text-slate-500">No content linked yet. Multiple items of the same type are supported.</p>}</div>
