...
const PROVIDERS = {
  github: { label: 'GitHub', description: 'Repository context for delivery work. Release 1 is read and connection-test only.', secretPrefix: 'ANKA_GITHUB_' },
  figma: { label: 'Figma', description: 'Design-file context for reviews and handoffs. The OS cannot alter Figma files.', secretPrefix: 'ANKA_FIGMA_' },
  wordpress: { label: 'WordPress', description: 'Site delivery connection. Publishing remains human-controlled during UAT.', secretPrefix: 'ANKA_WORDPRESS_' },
}
...
function initialForm(provider = 'github') {
  return { provider, display_name: '', base_url: '', secret_name: PROVIDERS[provider].secretPrefix, owner: '', repo: '', file_key: '', username: '' }
}
...
function Status({ value }) {
  const style = value === 'verified' ? 'bg-emerald-950 text-emerald-300' : value === 'error' ? 'bg-red-950 text-red-300' : 'bg-slate-800 text-slate-300'
...
export default function Settings() {
  ...
  async function saveConnection(event) {
    ...
    try {
      const publicConfig = form.provider === 'github'
        ...
        : form.provider === 'figma' ? { file_key: form.file_key } : { username: form.username }
      await integrations.save({ provider: form.provider, display_name: form.display_name, base_url: form.base_url, secret_name: form.secret_name, public_config: publicConfig })
      setMessage('Connection metadata saved. Configure the named secret in Supabase before testing.')
  ...
  async function testConnection(connection) {
    ...
    try {
      ...
      setMessage(`${PROVIDERS[connection.provider].label} verified in ${result.latency_ms} ms.`)
      await loadConnections()
    } catch (testError) {
  ...
  return (
    <div className="h-full overflow-y-auto bg-slate-950 text-white">
      <header className="border-b border-slate-800 px-6 py-5">
        ...
        <h1 className="mt-1 text-2xl font-semibold tracking-tight">Secure Integrations</h1>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">Connection details live in the OS; credentials live only in Supabase Edge Function secrets. External writes remain disabled during UAT.</p>
      </header>
      ...
      <main className="mx-auto max-w-7xl space-y-6 px-6 py-6">
          ...
          {Object.entries(PROVIDERS).map(([id, provider]) => {
            ...
            return (
              <button key={id} type="button" onClick={() => chooseProvider(id)} className={`rounded-2xl border p-5 text-left transition ${form.provider === id ? 'border-purple-500 bg-purple-950/20' : 'border-slate-800 bg-slate-900/70 hover:border-slate-700'}`}>
                <div className="flex items-center justify-between"><h2 className="font-semibold">{provider.label}</h2><span className="text-xs text-slate-500">{verified}/{providerConnections.length} verified</span></div>
        ...
        <div className="grid gap-6 xl:grid-cols-[1fr_380px]">
          <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
            <div className="flex items-center justify-between gap-3"><div><h2 className="font-semibold">Configured connections</h2><p className="mt-1 text-sm text-slate-500">Tests return limited metadata and never return provider credentials.</p></div><span className="text-xs text-slate-500">{connections.length} total</span></div>
            <div className="mt-5 space-y-3">
              {loading ? <p className="text-sm text-slate-500">Loading integrations…</p> : connections.length === 0 ? (
                <div className="rounded-xl border border-dashed border-slate-700 px-5 py-10 text-center text-sm text-slate-500">No Release 1 integrations configured yet.</div>
              ) : connections.map((connection) => (
                <article key={connection.id} className="rounded-xl border border-slate-800 bg-slate-950/50 p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-medium text-white">{connection.display_name}</p><p className="mt-1 text-xs text-slate-500">{PROVIDERS[connection.provider].label} · {connection.secret_configured ? 'Secret configured' : 'Secret missing'}</p></div><Status value={connection.status} /></div>
          ...
          <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
            <h2 className="font-semibold">Add {PROVIDERS[form.provider].label}</h2>
            {!canManage && !loading ? <p className="mt-4 text-sm leading-6 text-amber-300">Only system owners, operations admins, and executives can configure integrations.</p> : (
              <form onSubmit={saveConnection} className="mt-5 space-y-4">
                <label className="block text-xs font-semibold uppercase tracking-[0.12em] text-slate-400">Connection name<input required value={form.display_name} onChange={(event) => setForm({ ...form, display_name: event.target.value })} className={`${INPUT} mt-2 normal-case tracking-normal`} placeholder="Primary delivery connection" /></label>
                ...
                <button disabled={saving} className="w-full rounded-xl bg-purple-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-purple-500 disabled:opacity-50">{saving ? 'Saving…' : 'Save metadata'}</button>
              </form>
            )}
          </section>
        </div>
