...
export default function InternalWorkspace() {
  ...
  return <main className="min-h-full bg-[#090c13] p-4 text-slate-100 sm:p-6 lg:p-8"><div className="mx-auto max-w-[1600px]">
    ...
    {setupResult && <div role="status" className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-emerald-400/25 bg-emerald-500/10 p-4 text-sm text-emerald-100"><p>Internal Work draft saved{setupResult.replayed ? ' from the original confirmed request' : ''}. {setupResult.manager_binding_id ? 'The assigned project manager can activate it.' : 'Assign a project manager before activation.'}</p><button type="button" onClick={() => navigate(`/sphere/workspace/projects/${setupResult.project_id}`)} className="rounded-lg border border-emerald-200/20 px-3 py-2 font-medium outline-none focus:ring-2 focus:ring-emerald-300">Open project workspace</button></div>}
