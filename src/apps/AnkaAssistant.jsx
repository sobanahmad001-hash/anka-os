...
export default function AnkaAssistant() {
  ...
  async function runAssistant(event) {
    ...
    try {
      ...
      const response = await aiRepository.run({ capability, projectId: projectId || null, departmentId, input })
      setResult(response)
      setRuns(current => [{
  ...
  return <div className="min-h-full bg-slate-950 text-white">
    ...
    {activeTab === 'assistant' ? <main className="mx-auto grid max-w-7xl gap-6 p-6 xl:grid-cols-[360px_1fr]">
      <form onSubmit={runAssistant} className="h-fit space-y-5 rounded-2xl border border-slate-800 bg-slate-900/70 p-5"><div><h2 className="font-semibold">Request assistance</h2><p className="mt-1 text-xs text-slate-500">The server retrieves only records your session can access and uses the verified connector assigned to the operating department.</p></div><Field label="Capability"><div className="space-y-2">{CAPABILITIES.map(([id, label, description]) => <button type="button" key={id} onClick={() => setCapability(id)} className={`w-full rounded-xl border p-3 text-left ${capability === id ? 'border-purple-600 bg-purple-950/40' : 'border-slate-800 bg-slate-950/40'}`}><p className="text-sm font-medium">{label}</p><p className="mt-1 text-xs text-slate-500">{description}</p></button>)}</div></Field><Field label="Project context"><select className={INPUT} value={projectId} onChange={event => setProjectId(event.target.value)}><option value="">My work only</option>{projects.map(project => <option key={project.id} value={project.id}>{project.name}</option>)}</select></Field><Field label="Operating department"><select required className={INPUT} value={departmentId} onChange={event => setDepartmentId(event.target.value)}><option value="">Select department</option>{departmentOptions.map(([id, label]) => <option key={id} value={id}>{label}</option>)}</select></Field><Field label={capability === 'project_pulse' ? 'Specific focus (optional)' : 'Request'}><textarea className={INPUT} rows="5" value={input} onChange={event => setInput(event.target.value)} placeholder={placeholder(capability)} /></Field><button disabled={running || !departmentId || (projectId && !workspace) || (!projectId && capability !== 'daily_brief')} className="w-full rounded-xl bg-purple-600 px-4 py-2.5 text-sm font-semibold disabled:opacity-50">{running ? 'Analyzing authorized context…' : `Run ${selectedCapability?.[1]}`}</button></form>
