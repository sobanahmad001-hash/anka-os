...
export default function ContentStudio() {
  ...
  return <div className="h-full overflow-y-auto bg-slate-950 text-white">
    ...
    <main className="mx-auto max-w-7xl space-y-6 px-6 py-6">
      ...
      {!repositories ? <div className="py-20 text-center text-sm text-slate-500">Select an active organization to open Content Studio.</div> : tab === 'general' ? <GeneralContentRequestsPanel key={`${activeOrganizationId}:${scopeRevision}`} repository={repositories.requests} /> : tab === 'library' ? <ContentLibraryPanel key={`${activeOrganizationId}:${scopeRevision}`} repository={repositories.studio} /> : tab === 'home' ? <ContentWorkshopHomePanel key={`${activeOrganizationId}:${scopeRevision}:${engagementId}`} workspace={workspace} loadState={loading ? 'loading' : workspaceDenied ? 'denied' : error && !workspace ? 'error' : 'ready'} stale={workspaceStale} error={error} onRetry={() => loadWorkspace(engagementId)} onNewContent={() => selectTab('writer')} onOpenBrief={() => selectTab('brand')} onOpenEditor={(item, editorTab) => { setType(item.contentType); selectTab(editorTab) }} /> : loading ? <div className="py-20 text-center text-sm text-slate-500">Loading Content Studio…</div> : tab === 'queue' ? <ContentQueuePanel key={`${activeOrganizationId}:${scopeRevision}`} repository={repositories.queue} /> : !workspace ? <div className="rounded-2xl border border-dashed border-slate-700 px-6 py-16 text-center text-sm text-slate-500">Activate a Content service on an engagement to begin, or use General requests without an engagement.</div> : tab === 'writer' ? (
