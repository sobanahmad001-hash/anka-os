...
import { websiteSitemapPreview } from '../data/contentArchitectureReview.js'
import WebsitePageBriefEditor from '../components/WebsitePageBriefEditor.jsx'
...
function ArtifactWorkspace({ studio, customFields, workspace, type, setType, saving, act, onRefresh, originLinkId }) {
  ...
  return <div className="grid gap-6 xl:grid-cols-[320px_1fr]">
    <section className="space-y-3">{Object.entries(CONTENT_ARTIFACT_FORMS).map(([id, definition]) => {
      ...
      <ArtifactForm key={`${workspace.engagement.id}:${type}:${latest?.id || 'new'}`} studio={studio} customFields={customFields} workspace={workspace} type={type} artifact={artifact} versions={versions} latest={latest} approval={approval} saving={saving} act={act} onRefresh={onRefresh} originLinkId={originLinkId} />
  </div>
}
...
function ArtifactForm({ studio, customFields, workspace, type, artifact, versions, latest, approval, saving, act, onRefresh, originLinkId }) {
  ...
  async function save(event) {
    event.preventDefault()
    if (currentOutline && !outlineReady) {
      setOutlinePreview(currentOutline)
      return
    }
  ...
  const briefSourceTypes = new Set(['discovery', 'vision', 'audience', 'brand_statement', 'content', 'keyword_strategy'])
  const briefArtifacts = new Map(workspace.artifacts.map(item => [item.id, item]))
  const briefVersions = workspace.versions.filter(version =>
    briefSourceTypes.has(briefArtifacts.get(version.artifact_id)?.artifact_type))
  ...
  return <div><form onSubmit={save} className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6">
    ...
    <div className="mt-6 flex flex-wrap justify-end gap-3 border-t border-slate-800 pt-5"><button disabled={saving || blockingIssues || (currentOutline && !outlineReady)} className={PRIMARY}>{saving ? 'Saving…' : currentOutline ? 'Apply reviewed structure draft' : latest ? 'Create new version' : 'Save first version'}</button></div>
...
function ArtifactField({ field, value, pageSlugs, architectureVersions, selectedArchitectureVersion, contentRequests, recordErrors, recordWarnings, briefEnabled, briefVersions, briefArtifacts, onChange }) {
...
function RecordEditor({ index, field, records, pageSlugs, record, pathError, briefEnabled, briefVersions, briefArtifacts, onChange, onAddChild, onDuplicate, onMove, onRemove }) {
  return <div className="rounded-2xl border border-slate-800 bg-slate-950/50 p-4"><div className="mb-4 flex items-center justify-between gap-3"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-amber-400">{field.label} {index + 1}</p><div className="flex flex-wrap items-center justify-end gap-2">{field.recordType === 'website_page' && <WebsitePageRecordActions label={record.title || `page ${index + 1}`} buttonClassName={BUTTON} canMoveEarlier={index > 0} canMoveLater={index < records.length - 1} onAddChild={onAddChild} onDuplicate={onDuplicate} onMoveEarlier={() => onMove(-1)} onMoveLater={() => onMove(1)} />}<button type="button" onClick={onRemove} className="text-xs font-semibold text-red-300">Remove</button></div></div><div className="grid gap-4 md:grid-cols-2">{field.recordFields.map(([key, label, kind, options]) => {
