...
function ProjectRow({ row, navigate }) {
  ...
  return <tr className="align-top text-slate-300"><td className="px-4 py-4"><div className="font-medium text-white">{row.name}</div><div className="mt-1 text-xs text-slate-500">{row.ownerKind === 'internal' ? 'Internal Work' : `Client Work${row.clientName ? ` · ${row.clientName}` : ''}`}{row.brandName ? ` · ${row.brandName}` : ''}</div>{row.engagementId && <button type="button" onClick={() => navigate(`/sphere/engagements?engagement=${row.engagementId}`)} className="mt-2 text-xs font-medium text-violet-300 hover:text-violet-200">Open engagement record →</button>}</td><td className="px-4 py-4"><span className="rounded-full border border-white/10 px-2 py-1 text-xs">{label(row.status)}</span><div className="mt-2 text-xs text-slate-500">{label(row.health)}</div></td><td className="px-4 py-4"><div>{row.owner.name}</div><div className="mt-1 text-xs text-slate-500">{row.dueDate || 'No due date'}</div></td><td className="px-4 py-4 text-xs">{taskText}</td><td className="px-4 py-4 text-xs"><div>{itemText}</div>{row.engagementWorkItems.automationFlags > 0 && <div className="mt-1 text-amber-300">{row.engagementWorkItems.automationFlags} automation flags</div>}</td><td className="max-w-xs px-4 py-4 text-xs">{row.attentionSignals.length ? <ul className="space-y-1 text-amber-200">{row.attentionSignals.map((signal) => <li key={signal}>• {signal}</li>)}</ul> : <span className="text-slate-600">None</span>}</td></tr>
}
