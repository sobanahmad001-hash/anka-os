...
export default function LivingProjectReference() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const data = await livingProjectReference.load(activeOrganizationId, projectId, { signal: requestSignal })
  ...
  }, [activeOrganizationId, projectId, requestSignal, scope, handleOrganizationAccessError])
  ...
  const preserve = async () => {
    ...
    try {
      await livingProjectReference.preserve({ organizationId: activeOrganizationId, projectId,
        ...
        reason: 'Project document core record checkpoint',
      }, { signal: requestSignal })
      if (current.current === requestedScope && !requestSignal?.aborted) {
        snapshotRequest.current = null; setNotice({ scope, signal: requestSignal, message: 'Core record checkpoint preserved. Supplementary services, pipelines and decisions are not included in this checkpoint.' }); await load()
      }
    } catch (cause) {
      if (current.current === requestedScope && !requestSignal?.aborted) {
  ...
  return <main className="min-h-full bg-slate-950 p-4 text-white sm:p-8"><div className="mx-auto max-w-6xl space-y-5">
    ...
    {doc && <>
      <Section title="Freshness & version history"><p className="text-sm text-slate-400">Current records loaded {date(doc.loadedAt)} · core source v{doc.sourceVersion}. Refresh reads recorded changes; this is not a live conversation feed.</p><p className="mt-2 text-xs text-amber-200">{doc.coverage}</p>{doc.unavailable.length > 0 && <p className="mt-2 text-xs text-amber-200">Unavailable to this view: {doc.unavailable.join(', ')}. No agreement is inferred from missing data.</p>}<div className="mt-4 flex flex-wrap items-center gap-3"><label className="text-sm">View <select aria-label="Document version" className="ml-2 rounded-lg bg-slate-900 p-2" value={snapshotId || ''} onChange={event => selectHistory(event.target.value)}><option value="">Current composed reference</option>{doc.snapshots.map(row => <option key={row.id} value={row.id}>{label(row.projection_kind)} core v{row.source_version} · {date(row.generated_at)}</option>)}</select></label><button className={button} onClick={preserve} disabled={saving || loading || Boolean(snapshotId) || !canPreserveReportsSnapshot({ membership: activeMembership, userId: user?.id, projectOwnerId: doc.project.owner_id })}>{saving ? 'Preserving…' : 'Preserve core checkpoint'}</button></div></Section>
      {snapshotId ? snapshot ? <Section title={`Preserved ${label(snapshot.projection_kind)} core record v${snapshot.source_version}`}><p className="mb-3 text-sm text-slate-400">Generated {date(snapshot.generated_at)} · {snapshot.reason}. Only this saved projection is shown; current services or decisions are not merged into history.</p><pre className="max-h-[70vh] overflow-auto whitespace-pre-wrap break-words rounded-xl bg-black/20 p-4 text-xs text-slate-300">{JSON.stringify(snapshot.snapshot, null, 2)}</pre></Section> : <p role="alert" className="text-rose-300">This checkpoint is unavailable in the selected project. No current record is shown as history.</p> : <>
