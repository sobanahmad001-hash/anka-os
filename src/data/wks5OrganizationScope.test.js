...
test('WKS5 views wait for organization resolution and reset organization-bound state', () => {
  ...
  assert.match(myWork, /setWorkspace\(null\); setLoading\(true\); setSaving\(''\); setError\(''\)/)
  assert.match(myWork, /setVersionTarget\(null\)/)
  assert.match(myWork, /setReviewTarget\(null\)/)
  assert.match(myWork, /setSubmissionTarget\(null\)/)
  assert.match(myWork, /actionRequests\.current\.clear\(\)/)
  assert.match(myWork, /TABS\.some\(\(\[id\]\) => id === requestedTab\)/)
