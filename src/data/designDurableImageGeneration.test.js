...
test('provider ambiguity is terminal and cannot enter the confirmed-failure retry path', () => {
  ...
  assert.match(edge, /isConfirmedProviderRejection\(apiResponse\.status, apiError\)/)
  assert.match(edge, /status !== 400 \|\| text\(providerError\.type, 80\) !== 'invalid_request_error'/)
  assert.match(edge, /canRetryImageGenerationJob\(source as Json\)/)
