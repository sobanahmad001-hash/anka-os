...
export function createDepartmentChatRepository(client) {
  async function invoke(action, input = {}) {
    const envelope = await client.functions.invoke('department-chat', { body: { action, ...input } })
  ...
  return Object.freeze({
    getOfficialRecord: async (organizationId, decision) => {
      ...
      const envelope = await client.from(table).select('*').eq('organization_id', organizationId).eq('id', id).single()
      if (envelope.error || envelope.status >= 400) throw failure(envelope.error, envelope)
      return envelope.data
    },
    proposeArtifact: (departmentId, input) => invoke('propose_artifact', { ...input, department_id: departmentId }),
    proposeWorkItem: (departmentId, input) => invoke('propose_work_item', { ...input, department_id: departmentId }),
    confirmProposal: proposalId => invoke('confirm_proposal', { proposal_id: proposalId }),
    rejectProposal: proposalId => invoke('reject_proposal', { proposal_id: proposalId }),
  })
}
