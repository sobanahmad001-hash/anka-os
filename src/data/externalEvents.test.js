...
const edgeTest = read('../../supabase/functions/external-events/index.test.ts')
...
test('MK1 schema is organization-safe, indexed, and browser read-only', () => {
  ...
  assert.match(migration, /event_category text not null/)
...
test('external event server writes require an allowed department or leadership', () => {
  ...
  assert.match(edge, /event_category: eventCategory/)
  assert.match(edgeTest, /rejects the legacy category contract/)
  assert.match(ci, /deno test --frozen[^\n]*external-events\/index\.test\.ts/)
  assert.match(ci, /deno check --frozen[^\n]*external-events\/index\.ts[^\n]*external-events\/index\.test\.ts/)
})
