...
test('M03 preserves calendar projection and uses a date-only authenticated scheduling command', () => {
  ...
  assert.match(repository, /schedule_marketing_calendar/)
  assert.doesNotMatch(repository, /\.insert\(|\.update\(|\.upsert\(|\.delete\(|rpc\(/)
  assert.match(component, /Confirm date-only schedule/)
  assert.match(component, /server rechecks ownership, dependencies and the exact row version/)
  ...
  assert.match(component, /draggable=\{schedulable\}/)
  assert.match(component, /onDropDay\(day\)/)
  assert.match(component, /Cancel<\/button>/)
  assert.doesNotMatch(component, />Publish</)
