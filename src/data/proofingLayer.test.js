...
test('comment content and targets are append-only while resolution is one-way', () => {
  assert.match(migration, /trg_artifact_version_comments_append_only/)
  ...
  assert.match(migration, /new\.design_direction_version_id is distinct from old\.design_direction_version_id/)
...
test('the reusable proofing panel covers Content, Marketing, and Design direction versions', () => {
...
test('comments remain exact-version scoped and are never copied on revision', () => {
