...
test('P5 is wired into Project Workspace without cross-type drag or browser date conversion', () => {
  ...
  assert.match(workItemsSource, /workItems\.remove\(organizationId, editor\.id, editor\.row_version\)/)
  assert.match(workItemsSource, /workItems\.addDependency\(organizationId, editor\.id, dependencyCandidate, editor\.row_version\)/)
  assert.match(workItemsSource, /workItems\.removeDependency\(organizationId, workItemId, dependsOnWorkItemId, source\?\.row_version\)/)
  assert.match(workItemsSource, /workItems\.acknowledgeAutomationFlag\(organizationId, item\.id, item\.row_version\)/)
  assert.match(workItemsSource, /workItems\.move\(organizationId, item\.id, item\.row_version, targetStatus, beforeWorkItemId\)/)
