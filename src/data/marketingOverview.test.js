...
import { buildMarketingOverview, composeMarketingOverviewFamilies, filterMarketingOverview, marketingOverviewCounts, marketingOverviewLoadFailure, marketingOverviewRowHref } from './marketingOverview.js'
...
input.campaignLinks = [{ organization_id: 'org-a', campaign_id: 'campaign-a', artifact_id: 'artifact-a' }]
...
const navigation = { organizationId: 'org-a', clientId: 'client-a', projectId: 'project-a', engagementId: 'eng-a', brandId: 'brand-a', serviceId: 'service-a' }
...
test('row drilldowns preserve complete authorized context and exact record identity', () => {
  ...
  for (const kind of ['project_task', 'engagement_work_item']) {
    ...
    assert.deepEqual(Object.fromEntries(['ctxOrg', 'ctxClient', 'ctxProject', 'ctxEngagement', 'ctxBrand', 'ctxService'].map(key => [key, url.searchParams.get(key)])), {
      ctxOrg: 'org-a', ctxClient: 'client-a', ctxProject: 'project-a', ctxEngagement: 'eng-a', ctxBrand: 'brand-a', ctxService: 'service-a',
    })
    assert.equal(url.searchParams.get('campaign'), null)
  ...
  assert.equal(reviewUrl.searchParams.get('campaign'), 'campaign-a')
})
...
test('Overview UI labels failed refresh data as stale and preserves partial-family disclosure', () => {
  ...
  assert.match(repository, /composeMarketingOverviewFamilies\(settled/)
})

test('membership-only failure preserves healthy work without authoritative unassigned labels', async () => {
  let profileCalls = 0
  const result = await composeMarketingOverviewFamilies([
    { status: 'fulfilled', value: input.tasks }, { status: 'fulfilled', value: input.workItems },
    { status: 'fulfilled', value: input.approvalRequests }, { status: 'rejected', reason: new Error('memberships unavailable') },
    { status: 'fulfilled', value: input.campaignLinks },
  ], async () => { profileCalls += 1; return [] })
  const overview = buildMarketingOverview({ ...input, ...result })
  assert.equal(profileCalls, 0)
  assert.equal(result.sectionErrors.memberships, 'memberships unavailable')
  assert.equal(overview.records.find(item => item.recordKind === 'project_task').ownerLabel, 'Owner unavailable')
  assert.equal(filterMarketingOverview(overview.records, { facet: 'due_work', owner: '__unassigned__' }).some(item => item.recordId === 'task-a'), false)
})

test('profile-only failure remains partial and marks names unavailable', async () => {
  const result = await composeMarketingOverviewFamilies([
    { status: 'fulfilled', value: input.tasks }, { status: 'fulfilled', value: input.workItems },
    { status: 'fulfilled', value: input.approvalRequests }, { status: 'fulfilled', value: input.memberships },
    { status: 'fulfilled', value: input.campaignLinks },
  ], async () => { throw new Error('profiles unavailable') })
  const overview = buildMarketingOverview({ ...input, ...result })
  assert.equal(result.sectionErrors.profiles, 'profiles unavailable')
  assert.equal(overview.records.find(item => item.recordKind === 'project_task').ownerLabel, 'Owner name unavailable')
  assert.equal(overview.records.some(item => item.recordKind === 'project_task'), true)
})
