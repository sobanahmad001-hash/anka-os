...
test('mounted MB05 calendar switches views and filters real rendered cards without scheduling writes', async t => {
  ...
  const monthCard = byText(container, 'article', 'Email work')
  assert.match(monthCard.textContent, /2026-09-10/)
  assert.match(monthCard.textContent, /Amina/)
  assert.match(monthCard.textContent, /Email.*Launch/)
  assert.match(monthCard.textContent, /1 unresolved prerequisite/)
  assert.match(monthCard.textContent, /1 prerequisite status unknown/)
  await change(byLabel(container, 'Marketing calendar month'), '2026-10')
  assert.match(container.textContent, /No scheduled work matches this month and filters/)
  await change(byLabel(container, 'Marketing calendar month'), '2026-09')
