...
test('PLN3 preview is read-only and creation is stale-safe atomic and replay-safe', () => {
  ...
  assert.match(compose, /lock table public\.blueprint_stage_catalog,[\s\S]*public\.service_stage_rules in share mode/)
  ...
  assert.match(migration, /Canonical prerequisite stage selection is ambiguous/)
  assert.match(migration, /Canonical context dependency reason is ambiguous/)
  assert.match(migration, /order by candidate\.display_order desc\s+limit 1/)
  assert.doesNotMatch(migration, /order by candidate\.display_order desc, candidate\.stage_id/)
...
test('PLN3 UI keeps direct composition and gates template creation on a current preview', () => {
  ...
  assert.match(operatingSpine, /Promise\.allSettled/)
  assert.match(operatingSpine, /templateResult\.status === 'fulfilled'/)
