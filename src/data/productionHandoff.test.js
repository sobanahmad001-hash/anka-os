...
test('the DS6 verifier is named, rollback-safe, and never commits', () => {
  ...
  assert.match(verifier, /tgtype = 19[\s\S]*tgenabled <> 'D'/)
  ...
  assert.match(verifier, /idx_production_handoff_packages_preparing[\s\S]*indisvalid[\s\S]*indisready[\s\S]*indnkeyatts = 2[\s\S]*indnatts = 2[\s\S]*indexprs is null[\s\S]*amname = 'btree'/)
