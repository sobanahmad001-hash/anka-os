...
export function createMarketingCalendarRepository(organizationId, { signal, client = supabase } = {}) {
  ...
  return Object.freeze({ async load(engagement) {
    ...
    const [clientRecord, tasks, workItems, campaigns, planVersions, campaignLinks, memberships] = await Promise.all([
    ...
    const [dependencyTasks, dependencyWorkItems] = await Promise.all([
      dependencyTaskIds.length ? dataOrThrow(client.from('tasks').select('id, organization_id, project_id, status').eq('organization_id', organizationId).eq('project_id', project.id).in('id', dependencyTaskIds), options) : [],
      dependencyWorkItemIds.length ? dataOrThrow(client.from('work_items').select('id, organization_id, project_id, engagement_id, status').eq('organization_id', organizationId).eq('project_id', project.id).eq('engagement_id', engagement.id).is('deleted_at', null).in('id', dependencyWorkItemIds), options) : [],
    ])
    return buildMarketingCalendar({ organizationId, engagement, project, client: clientRecord, tasks, workItems, taskDependencies, workItemDependencies, dependencyTasks, dependencyWorkItems, campaigns, planVersions, campaignLinks, memberships, profiles })
  } })
}
