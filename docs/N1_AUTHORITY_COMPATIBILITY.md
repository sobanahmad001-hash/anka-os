...
## N1-D — scoped deactivation and non-destructive invitation recovery

Dependency: `42a613dae5f3fb3315dfbdb2dde35bf53ca300fb`. Master relayed explicit user approval to replace routine Auth deletion with organization-scoped deactivation, preserve history, and reserve permanent deletion/anonymization for separate review. Master additionally confirmed that successful automatic failed-invite Auth deletion is **not required**: conservative `cleanup_incomplete` / manual recovery is the approved boundary. No global deletion design was added.

### Implemented boundary

- New authenticated deactivation RPC uses current System Owner/Operations Admin authority, active organization scope and the existing per-organization administration lock. It locks the same-org team target, rejects self-removal and protects the last active System Owner and last organization administrator. Replays reauthorize current authority and reject changed request payloads. Repeat deactivation does not duplicate the immutable audit event.
- Membership is retained with `status=revoked`. Only that organization's active department affiliations, contributor designations, PM bindings and recurring delegations approved by the target are revoked; history is not deleted. Other organizations' memberships/bindings, global profile, Auth identity and Auth sessions are untouched.
- Open Project Task and Engagement Work Item assignments remain intact with their original assignees/versions. An immutable private deactivation audit captures prior membership, actor/request, revoked-authority counts and reassignment snapshots for both work types. Admin-only history controls expose those snapshots for review, explicitly not as a claim that an old assignment remains current. No work is erased or silently unassigned.
- The old `invite-user` DELETE branch now performs fixed-Anka scoped deactivation, as does the new explicit POST action. It cannot globally delete a user. Caller organization overrides are rejected; actor identity comes from verified getUser and the authenticated database connection. Current database authority is rechecked inside the command. Self-removal remains denied.
- Selected-organization admin UI provides explicit confirmation and retained reassignment history; legacy fixed-Anka controls now say Deactivate Anka access and retain revoked members instead of removing them from the list. The legacy list excludes profiles with no Anka membership. No old routine Delete/Remove handler calls Auth deletion.
- Invitation completion is an atomic authenticated fixed-Anka membership insert, validating current admin, matching Auth identity/email and same-org department. It cannot overwrite/reactivate an existing membership. New display profiles remain the existing Auth trigger's responsibility; global profile role/department are no longer overwritten by this endpoint, so other-org identity/authority is not changed.
- Both known setup failure and uncertain completion explicitly report a retained Auth account and incomplete cleanup/manual recovery. Provider errors/details are not exposed; no blind compensation or automatic reinvite/retry occurs. Residual invited Auth accounts are an operational recovery item, **not successful cleanup**. No deleteUser, ban, sign-out or credential mutation remains in invite-user.

No pre-existing last-owner/admin safeguard was found in the inspected source; N1-D explicitly protects both for its deactivation command. Unrelated legacy role-edit APIs were not redesigned. The separate invite-client-contact compensation path was observed but is outside the approved invite-user slice and was not changed or claimed safe.

### Focused verification

- Selective synthetic localhost SQL passed: deactivate/replay/new-request repeat; wrong org/target/non-admin/self denial; immutable retained membership/history; derived authority/delegation revocation; both assignment snapshots with unchanged work; shared Auth/profile and other-org membership/PM access retained; atomic fixed-Anka invitation success, failure with no partial membership, and existing-member overwrite denial.
- Using the same synthetic authenticated identity before/after (no token refresh), revoked-org capability/read/write boundaries reject access while other-org capabilities remain usable. Service P5 execution also rejects the revoked actor. This proves current database membership checks in the fixture, **not live Auth JWT/session revocation or full signed-in acceptance**. Responses explicitly report `sessions_revoked=false`.
- Two real sessions passed competing admin deactivations: current-authority recheck rejects the losing revoked admin, one administrator survives, self-removal is denied, replay retains one audit event, and other-org administration remains active.
- Final last-System-Owner delta reused the retained cluster: removing the sole owner is denied even to an active Operations Admin; deactivation succeeds when a second active owner exists. Final localhost security advisor: No issues found.
- Six mocked Deno invite-user endpoint tests passed using cached/frozen dependencies; no actual email/provider call occurred. Two changed Node provisioning contracts passed; four new mounted deactivation UI tests passed. Unchanged suites reused.
- Build passed (477 modules; existing chunk warning); scoped lint 0 errors, 7 JSX-used-name warnings. Retained stopped SQL evidence: `G:/AnkaSphereN1LocalChecks/anka-n1-b56107c1403740d2bc24c771467ed590`.

### Review gate

The previously recorded routine invite-user hard-delete/history incompatibility is removed by deactivation and non-destructive failed-invite recovery. Permanent deletion/anonymization and residual-account cleanup require the separate reviewed process; no FK/history erasure was attempted. Installed ACL/full-schema and real signed-in integration gates remain. Specialist review versus PM confirmation versus release is still the next separate enforcement scope, with no automatic new powers.

Supabase security skills guided current database membership authorization, private fixed-search-path functions, explicit ACLs, honest session limitations and localhost advisors. Current [deleteUser](https://supabase.com/docs/reference/javascript/auth-admin-deleteuser), [inviteUserByEmail](https://supabase.com/docs/reference/javascript/auth-admin-inviteuserbyemail) and [changelog](https://supabase.com/changelog) references were checked. No relevant listed change requires changing this conservative boundary. No publication, live migration, production fixture, provider or paid action occurred.

## N1-E — governed approval separation (local, release blocked)

This local candidate makes the previously deferred distinction explicit without changing P7's lifecycle-state vocabulary: a specialist review, an exact-project PM confirmation, and a client release are three separate decisions. The new immutable PM-confirmation record is exact-version, request-idempotent, checks the expected version state, and rejects author/owner/reviewer self-confirmation. No existing P7 release is newly blocked by this record: routing confirmation into a release gate requires a separately approved consumer scope. Exact PM bindings are cross-department confirmation authority only; they do not become specialist reviewers or release authority. System Owner, Operations Admin, and the existing elevated Executive role retain their existing authority, while department heads require both canonical `department_manager` membership and active project-department participation. Contributor designations, including the N1 `executive` designation, grant no approval authority.

P7 reviewer candidates, assignment, review, capability responses and release authority now share those scopes. The My Work release UI exposes server-provided PM confirmation separately from the release action; it does not infer capability from browser roles.

Artifact approval request nomination, sign-off and change-request commands now reauthorize the actor against the artifact's canonical project at each decision. Being named remains necessary but is no longer sufficient. Because artifact rows have no canonical department field, the migration uses an explicit artifact-type-to-department mapping and fails closed to leadership for unknown types; it never falls back to global profile roles. Existing artifact versions remain immutable and change comments remain append-only.

Focused adapter and multi-approver policy tests passed, as did the production build (477 modules; existing chunk-size warning) and `git diff --check`. This is source/build validation only: a synthetic SQL/concurrency harness for the full P7/artifact parent graph, installed-schema compatibility, and signed-in integration remain release gates. No migration was applied outside local source, and no live/provider action occurred.

### N1-E focused local SQL follow-up

The N1-E runner applies the real N1-E migration after the existing N1/N1-B/N1-C authority foundations. It loads the actual P7 review-assignment, lifecycle-event and action-request table definitions, the P7 release/receipt/team functions, and the artifact multi-approver table definitions from their migrations. Only the remaining older delivery and artifact parents are explicit synthetic fixtures; this is not a full historical replay. A localhost PostgreSQL 17.11 run passed: exact-project PM confirmation and replay/conflict, PM cannot release, real P7 release without a PM confirmation, revoked-PM replay denial, service-role artifact nomination/sign-off/change requests, and participation revocation blocking a pending head sign-off. The two-session test confirmed that a concurrent retry waited on the first uncommitted confirmation, produced one record and an idempotent replay, then rejected replay after binding revocation. Local security advisor reported no issues. The final cluster was stopped and retained at `G:/AnkaSphereN1LocalChecks/anka-n1-2a65e8d0987643dcbf0786810514a866`.

This run exposed and fixed a migration defect: N1-E's service-role artifact RPCs were denied EXECUTE on their private authority helpers. The migration now grants only those read-only helpers to `service_role`; `anon` and `authenticated` remain denied. Node syntax and diff checks passed. Installed parent lock ACLs/role attributes, full installed-schema compatibility, and real signed-in integrated acceptance remain unverified release gates. No live migration, provider call, push, or deployment occurred.

### Installed baseline metadata preflight — 2026-09-20 (read only)

The connected `anka-os` Supabase project's installed migration ledger has 95 entries and matches every pre-N1 migration version in this checkout. Exactly seven N1 migrations remain local only, in the intended order. Read-only catalog queries found all 19 exact parent tables and every column checked for N1-E, plus the expected signatures and return types for eight P7/artifact functions. Existing composite unique keys needed by N1-E are present on projects, deliverable versions, organization memberships, artifact versions and artifact approval requests. The department `(id, organization_id)` key is not installed yet; the first pending N1 migration explicitly creates it.

Installed `postgres` and `service_role` are non-superuser BYPASSRLS roles. `service_role` has `public`/`private` schema USAGE and SELECT plus a column UPDATE privilege on each sampled installed lock parent: organizations, organization memberships, projects, departments, engagements, deliverable versions, artifact versions, and artifact approval requests/signoffs. `authenticated` has `private` schema USAGE. This closes the previously unknown installed parent lock ACL/role-attribute preflight for those sampled dependencies, without granting anything live. Catalog shape and ACLs do not prove that all seven pending migrations apply cleanly to the full installed schema, that live backfill yields the expected records, or that signed-in flows pass. Those remain release gates. No data rows, credentials or secrets were queried or exposed, and no live write occurred.

### Installed public/private schema application — 2026-09-20 (local only)

With explicit approval, a schema-only export of the connected project's `public` and `private` schemas was restored into a disposable local PostgreSQL 17.11 database. The export contained 235 table definitions, 250 function definitions and 339 policies, with no `INSERT` or `COPY` data statements. Local synthetic `auth` users/functions and Supabase roles supplied only the managed-platform dependencies omitted by the scoped export. All seven pending N1 migrations applied in order to this restored schema without SQL errors. Post-application catalog assertions found the N1-E confirmation table/RLS, the expected anonymous/authenticated/service-role command and helper grants, the task/work/participation/PM-history guards, and empty-state membership/backfill behavior. The local Supabase security advisor reported no issues.

This closes SQL application compatibility against the installed `public`/`private` schema shape under the stated local stubs. It does not prove managed Supabase platform behavior, production-row backfill, or signed-in integrated acceptance. The export and restore made no live database changes; all seven migrations remain unapplied remotely. Release stays blocked on those remaining acceptance gates.

### Live backfill aggregate preflight — 2026-09-20 (read only)

Aggregate-only queries against the connected project counted one organization, one membership, four departments, two projects and two engagements. Applying the first migration's exact selection predicates predicts one legacy department-membership seed, two verified project-manager binding seeds, zero reconciliation issues and zero duplicate seed IDs. No row values or identities were retrieved. This is an eligibility estimate, not a live backfill result. The first migration explicitly locks five parent tables in SHARE mode and adds a department uniqueness constraint; its 5-second lock timeout and 60-second statement timeout bound waiting and individual statement time, but not the full migration's write interruption. No live dry run or migration was executed.
