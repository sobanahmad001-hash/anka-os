...
- The exact table ACL verifier accounts for PostgreSQL 17 `MAINTAIN` on `service_role` while continuing to require no anonymous grants and authenticated `SELECT` only.
